...
| # | 来源 | Issue | 标题 | 描述 | 预估收益 |
|---|------|-------|------|------|----------|
| T1 | opencode | #39294 | system prompt 路径浪费 3.3K tokens/轮 | `<available_skills>` 包含完整绝对路径，每轮浪费约 3.3K tokens | 每轮节省 3.3K tokens |
| T2 | kilocode | #12628 | 长会话 compaction 对免费模型失败 | chunk-based compaction 不适配 token 限制小的免费模型，导致重试浪费 | 避免 compaction 重试浪费 |
| T3 | kilocode | #12615 | 自动 compaction 导致无限循环 | compaction 触发条件不当，陷入无限压缩循环 | 避免无限 token 消耗 |
| T4 | kimi-cli | #2472 | compaction 重新加载 system prompt 浪费 20K tokens | compaction 后重新发送完整 system prompt + AGENTS.md | 每次 compaction 节省 ~20K tokens |
| T5 | kimi-cli | #2525 | Goal 模式空操作持续消耗 token | Goal 模式在等待外部条件时持续发起 "continue" 调用 | 避免空操作 token 浪费 |
| T6 | claude-mem | #3443 | Observer 历史无限增长：单次调用 1M tokens | 观察者对话历史无界增长，导致单次 API 调用达 1M tokens | 避免指数级 token 增长 |
| T7 | claude-mem | #3274 | 子智能体观察污染主会话上下文 | 子智能体观察注入主会话，增加无关 tokens | 减少无效 context |
| T8 | gemini-cli | #28362 | Token 消耗无限循环 | 代理进入无限重试循环，消耗 token 但不产出 | 避免零产出 token 消耗 |
| T9 | gemini-cli | #28339 | 429 错误导致无限回退循环 | 429 限流导致无限重试，无超时无错误显示 | 避免限流时 token 浪费 |
| T10 | kimi-code | #2177 | 超大工具结果阻止去重提醒 | >50K 字符的工具结果阻止去重提醒到达模型 | 避免上下文膨胀 |
| T11 | MiMo-Code | #1855 | /btw 侧问题发送百万 token | 声称"无 context 成本"但发送完整会话历史，达 1M-1.5M tokens | 避免侧问题 token 爆炸 |
| T12 | MiMo-Code | #1854 | OOM 10-20GB：无界 summary.diffs | 长会话累积 100MB+/消息 diffs，导致 10-20GB 内存和崩溃 | 避免内存导致的崩溃重试 |

### 中优先级

| # | 来源 | Issue | 标题 | 描述 |
|---|------|-------|------|------|
| T13 | opencode | #39444 | GLM-5.2 prompt cache 失效 | 自定义/反向代理路径导致 prompt cache 从 512 跌到 0 |
| T14 | opencode | #39291 | compaction 发送变异 thinking block → 400 重试循环 | compaction 机制缺陷导致永久重试 |
| T15 | kimi-cli | #2465 | reasoning_effort: null 导致严格 API 拒绝 | thinking "off" 时序列化为 null，严格 API 拒绝 |
| T16 | kimi-cli | #2523 | compaction 后重新打开已完成任务 | compaction 后任务状态丢失，导致重复执行 |
| T17 | claude-mem | #3409 | 配置过滤选项未连接到查询 | 用户配置的过滤器从未生效 |
| T18 | claude-mem | #3163 | 标题级样板观察绕过 content_hash | 近重复观察涌入 recall 窗口 |
...
| T21 | kimi-code | #2257 | footer 显示累计 token 用量 | 用户无法感知消耗 |
...
| U3 | opencode | #39208 | glob/grep 工具无超时 | 一个调用挂起 session 21 分钟 | 稳定性 |
...
| U11 | MiMo-Code | #1707 | 子智能体执行期间 TUI 冻结 | TUI 在子智能体执行时完全无响应 | TUI 稳定性 |
...
| # | 来源 | Issue | 标题 | 描述 | 分类 |
|---|------|-------|------|------|------|
| S1 | opencode | #39292 | Mac M4 多 session 系统冻结 | 并发使用导致系统冻结 | 稳定性 |
| S2 | opencode | #39463 | SQLite WAL 文件增长到 1GB+ | macOS 临时目录无限制增长 | 资源管理 |
| S3 | opencode | #39165 | /model 切换后 SQLite 崩溃 | 常用操作导致 session 损坏 | 稳定性 |
| S4 | opencode | #39456 | 子智能体工具调用静默中止 (macOS) | 自定义 provider 上子智能体失败 | 稳定性 |
| S5 | MiMo-Code | #1915 | Checkpoint rebuild 导致无限循环 | 完成任务后重新注入恢复提示 | 稳定性 |
| S6 | MiMo-Code | #1967 | 请求拒绝后永久挂起 | 拒绝权限请求后 TUI 无响应 | 稳定性 |
| S7 | MiMo-Code | #1746 | Plan 模式执行 bash 命令 | 计划模式应仅计划，不应执行 | 安全 |
...
| S11 | gemini-cli | #28271 | 事件驱动无限循环 | MAX_TURNS 限制无法阻止零延迟事件洪泛 | 稳定性 |
| S12 | claude-mem | #3404 | macOS 45GB 内存泄漏 | 203 个观察者子进程累积 4 天 | 资源管理 |
| S13 | claude-mem | #3340 | macOS sleep/wake 后 100% CPU | worker 守护进程在唤醒后 CPU 满载 | 资源管理 |
...
|--------|------|------|------|------|-----------|
| 🔴 P0 | T1 | opencode | system prompt 路径浪费 3.3K tokens/轮 | Token | 小 |
| 🔴 P0 | T4 | kimi-cli | compaction 重新加载 system prompt 浪费 20K tokens | Token | 中 |
| 🔴 P0 | U3 | opencode | glob/grep 工具无超时 | 稳定性 | 小 |
| 🔴 P0 | U11 | MiMo-Code | 子智能体执行期间 TUI 冻结 | 稳定性 | 中 |
| 🔴 P0 | S1 | opencode | Mac M4 多 session 系统冻结 | 稳定性 | 中 |
| 🟠 P1 | T2 | kilocode | 长会话 compaction 对免费模型失败 | Token | 中 |
| 🟠 P1 | T3 | kilocode | 自动 compaction 无限循环 | Token | 中 |
...
| # | 来源 | Issue | 标题 | 描述 | 分类 |
|---|------|-------|------|------|------|
| J1 | jcode | #651 | macOS stdin 检测常量错误 | `TH_STATE_WAITING` 应为 3 非 2，导致 stdin 管道输入完全失效 | macOS 核心 |
| J2 | jcode | #644 | 大上下文模型 compaction 永不触发 | 1M 窗口模型的 80% 阈值=800K，永不触发，每次重发完整历史 | Token |
| J3 | jcode | #540 | 长会话 TUI 输入和滚动退化 | 输入延迟增加、鼠标滚轮导致大量重绘 | TUI 性能 |
| J4 | jcode | #632 | TUI 粘贴文本 panic (critical) | `is_char_boundary` 断言失败导致崩溃 | 稳定性 |
| J5 | jcode | #617 | MCP 连接超时阻塞首次调用 30 秒 | 每次冷启动 30 秒无响应等待 | 启动体验 |
| J6 | jcode | #583 | 信息组件滚动时跳动闪烁 | 模型/上下文/用量信息在滚动时不稳定 | TUI 视觉 |
| J7 | jcode | #621 | macOS 菜单栏图标不显示 | 系统级交互体验问题 | macOS 特定 |
| J8 | jcode | #559 | API 429/5xx 重试不足 | 仅重试 3 次（~9 秒），之后直接失败 | 稳定性 |
| J9 | jcode | #570 | @file 文件提及 frecency 排序 | TUI 输入框 @ 文件自动补全 | 便捷操作 |
| J10 | jcode | #539 | /rewind 恢复编辑文件 | 回退对话时同时恢复文件编辑 | 工作流安全 |
| J11 | jcode | #629 | TUI 跟随终端 ANSI 配色 | 支持自定义主题，iTerm2 用户保持统一风格 | 个性化 |
...
| H2 | hermes-agent | #74003 | /model 命令响应 7+ 分钟 | 高频操作延迟严重退化 | 性能 |
| H3 | hermes-agent | #73772 | 模型静默回退无通知 | 429/502/超时时用户不知道回退了哪个模型 | 信息透明 |
| H4 | hermes-agent | #73678 | 子代理完成消息干扰可读性 | 完整结果逐字转储到会话历史 | 输出管理 |
| H5 | hermes-agent | #73702 | 禁用未使用 skill 注入系统提示词 | 155 个 skill 每轮注入 ~15K 字符 | Token |
| H6 | hermes-agent | #73825 | 模型列表含不存在的 3.x 模型 | 选择后静默 404 回退无提示 | 模型管理 |
| H7 | hermes-agent | #73823 | Token 统计永久失效 (schema>=22) | 无法追踪 token 消耗 | Token |
| H8 | hermes-agent | #73848 | 就地压缩导致 session 无法结束 | state.db 无限增长 | 资源管理 |
| H9 | hermes-agent | #73891 | 上下文压缩器耗尽 provider 池 | 524 超时后暂停 60 秒不切换 provider | 稳定性 |
| H10 | hermes-agent | #73777 | 空 content 重试浪费 35-40 秒 | Anthropic 返回空 content 时无效重试 | Token |
| H11 | hermes-agent | #73748 | 限流时 turn 丢失不重试 | 用户消息被静默丢弃 | 稳定性 |
| H12 | hermes-agent | #73680 | 多实例模型切换互相污染 | 一个实例切换模型影响另一个实例 | 会话隔离 |
...
| O2 | openclaw | #115273 | Prompt cache 滑动窗口 ~0% 命中率 | 每 turn 以全额 token 重发前缀 | Token |
| O3 | openclaw | #115272 | 模型切换 reasoning drop 翻转 | 每次 failover 摧毁 prompt cache | Token |
| O4 | openclaw | #115546 | compaction 超时远低于 deadline | 大 session compaction 100% 失败 | Token |
| O5 | openclaw | #115437 | 支持 fast mode (2.5x 吞吐) | Anthropic fast mode 可加速响应 | 性能 |
| O6 | openclaw | #115372 | TUI 选择器原语 | 缺少 AskUserQuestion 等价物 | TUI 交互 |
| O7 | openclaw | #115826 | 插件渲染瞬态 TUI 运行状态 | 长操作中无实时进度反馈 | TUI 交互 |
| O8 | openclaw | #115387 | 可滚动文件查看器 | 大文件无法查看 | TUI 功能 |

### claude-code-best (claude-code-best/claude-code)

| # | 来源 | Issue | 标题 | 描述 | 分类 |
|---|------|-------|------|------|------|
| C1 | claude-code-best | #1307 | 按 Provider/模型配置上下文窗口 | 避免过早/过晚压缩 | Token |
...
| C4 | claude-code-best | #1302 | filter-map 单次遍历优化 | 渲染路径性能优化 | 性能 |
...
| C8 | claude-code-best | #410 | 大段用户输入折叠显示 | 提升 TUI 可读性 | TUI 交互 |
...
| 🔴 P0 | U11 | MiMo-Code | 子智能体执行期间 TUI 冻结 | 稳定性 | 中 |
| 🔴 P0 | S1 | opencode | Mac M4 多 session 系统冻结 | 稳定性 | 中 |
| 🔴 P0 | J2 | jcode | 大上下文模型 compaction 永不触发 | Token | 中 |
| 🔴 P0 | J4 | jcode | TUI 粘贴文本 panic (critical) | 稳定性 | 中 |
| 🔴 P0 | H7 | hermes-agent | Token 统计永久失效 | Token | 小 |
| 🔴 P0 | O1 | openclaw | totalTokens ~2x 真实上下文 | Token | 中 |
| 🟠 P1 | T2 | kilocode | 长会话 compaction 对免费模型失败 | Token | 中 |
| 🟠 P1 | T3 | kilocode | 自动 compaction 无限循环 | Token | 中 |
| 🟠 P1 | T5 | kimi-cli | Goal 模式空操作 token 消耗 | Token | 小 |
| 🟠 P1 | U1 | opencode | session 切换性能 | 性能 | 中 |
...
| 🟠 P1 | H5 | hermes-agent | 禁用未使用 skill 注入 | Token | 小 |
...
| 🟡 P2 | U2 | opencode | Markdown 流式冗余高亮 | 性能 | 中 |
| 🟡 P2 | U9 | kimi-cli | 对话完成后滚动跳到底部 | 滚动行为 | 小 |
| 🟡 P2 | J3 | jcode | 长会话 TUI 输入滚动退化 | TUI 性能 | 中 |
| 🟡 P2 | H3 | hermes-agent | 模型静默回退无通知 | 信息透明 | 小 |
| 🟡 P2 | C5 | claude-code-best | OpenAI reasoning_effort | Token | 小 |
...
| L2 | #108 | 成本追踪示例 | loop-cost 命令缺少使用示例，无法帮助用户追踪和优化 token 消耗 | Token 可视化 | 用户感知消耗，主动优化 |
| L3 | #153 | 断路器防止无限循环 | loop-context circuit breaker 在重复失败或达到最大尝试次数时终止，防止 token 无限消耗 | Token 优化 | 避免无限重试浪费 token |
| L4 | #169 | 上下文管理 | loop-context 帮助管理会话上下文，避免上下文膨胀导致的 token 浪费 | Token 优化 | 减少无效上下文 token |
...
| L6 | #393 | 安全命令执行 | inputs.command 未正确引用，多参数/多行命令容易出错 | 安全 | 避免命令执行失败导致的重试 |
| L7 | #390 | 沙箱执行 | loop-sandbox 提供临时 worktree 隔离，安全执行修复命令 | 安全 | 安全试错，避免污染主分支 |
| L8 | #391 | 门控机制 | loop-gate 提供 denylist 和 auto-merge allowlist，控制自动化边界 | 安全 | 防止危险操作，提高信任度 |
| L9 | #139 | 参数验证 | 无效数值参数被静默接受，导致安全限制被绕过 | 稳定性 | 快速失败，避免错误配置导致的问题 |

### 按优先级汇总

| 优先级 | 编号 | Issue | 标题 | 类型 | 预估工作量 |
|--------|------|-------|------|------|-----------|
| 🔴 P0 | L5 | #408 | 并发 worktree 管理 | 稳定性 | 中 |
| 🔴 P0 | L1 | #246 | maker/checker 分离验证 | Token | 中 |
| 🟠 P1 | L3 | #153 | 断路器防止无限循环 | Token | 小 |
| 🟠 P1 | L7 | #390 | 沙箱执行 | 安全 | 中 |
| 🟠 P1 | L8 | #391 | 门控机制 | 安全 | 中 |
| 🟡 P2 | L6 | #393 | 安全命令执行 | 安全 | 小 |
| 🟡 P2 | L9 | #139 | 参数验证 | 稳定性 | 小 |
| 🟡 P2 | L4 | #169 | 上下文管理 | Token | 小 |
| 🟡 P2 | L2 | #108 | 成本追踪示例 | Token | 小 |
