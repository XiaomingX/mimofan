...
> 数据来源：`gh issue list --state open --limit 300`，共 **43** 个 OPEN issue（编号 598–777，跳跃为已关闭项）。逐项 `gh issue view` 取证，代码落地状态以 `git` 工作树 `grep`/读源码核验为准（参考 `SECURITY_CAPABILITY_PLAN.md`、`QUEUE_CAPABILITY_PLAN.md`）。本文只读梳理，不修改仓库、不改 issue 状态。
...
- **依赖/风险**：涉及主循环与所有工具签名，热路径改造；需与 #628/#726 可观测体系协同。

#### [L0-2] 记忆可观测 Stats（注入 token / last recall / 整合成本） `P1` `memory,#5`
- **来源 issue**：#628
- **问题/动机**：记忆系统无任何维度统计，无法判断召回质量/注入成本/陈旧度。
- **可实施范围**：新增 `MemoryStats`（注入 token 数、last recall 时间、整合次数/成本、条目数），在 status/debug 命令暴露，并供 #726 OTel 指标桥接。
...
- **依赖/风险**：需定义"关键事实"提取器（与 #777 评测归因协同）。
...
- **依赖/风险**：测试基建，不阻塞功能；与 #647 拆分同属可维护性债。
...
- **依赖/风险**：重构核心上下文路径，需回归测试护航（依赖 #635）。
...
#### [L1-1] 经验学习闭环（会话结束自动蒸馏回写记忆） `P1` `memory,A1,#659`
- **来源 issue**：#659（合并 #664 PTC、#661 /learn、#660 Curator、#669 rollout 导出、#672 blueprint 同属 Epic #658 经验/训练闭环族，见合并说明）
- **问题/动机**：写记忆唯一路径是 `remember` 工具（模型主动调用），无会话结束自动总结/回写机制。
- **可实施范围**：会话/任务结束后自动蒸馏经验（成功路径/踩坑/修复模式）回写为可检索记忆（`Observation`）；与 #732 UserProfile 注入协同。
- **验收口径**：跑完一个任务后，相关经验可被跨会话召回（端到端测试）；回写不污染主上下文。
- **依赖/风险**：依赖记忆子系统与 #732；需防回写噪声（与 #716 衰减协同）。

#### [L1-2] 用户建模层 UserProfile（跨会话用户画像） `P1` `memory,#732`
- **来源 issue**：#732
...
- **依赖/风险**：切片 A 已落地，剩 B–D；注入需 token 预算约束（与 #628 协同）。

#### [L1-3] 记忆巩固与遗忘（重要性/衰减/淘汰/去重/rollup） `P1` `memory,M4,#716`
- **来源 issue**：#716（与 #679 记忆模块合并、#714 混合检索、#628 可观测同属记忆体系）
...
- **依赖/风险**：切片 A 已落地；去重逻辑需收敛到单一实现（与 #679 协同）。

#### [L1-4] 混合检索四路融合（向量+全文+时近+重要性） `P1` `memory,M3,#714`
- **来源 issue**：#714
...
- **依赖/风险**：切片 A/B 已落地；真实语义 embedding 接入（见 #777 结论）是最大杠杆，但属独立配置项。

#### [L1-5] 合并分散记忆/去重代码到统一 mimofan-memory `P1` `refactor,memory,#679`
- **来源 issue**：#679
...
#### [L1-6] provider 电路熔断（Circuit Breaker）收尾接线 `P0` `reliability,#42,#619`
- **来源 issue**：#619
...
- **可实施范围**：将熔断状态机挂到 `advance_fallback` 的 readiness 过滤：Open 状态 provider 视为不可选；可恢复错误→对应 provider 失败计数+1，成功→重置；达阈值置 Open 并 cooldown；暴露 `circuit_breaker_status()` 指标（呼应 #637/#628）。
...
#### [L1-7] 统一次级模型（secondary_model）抽象收敛 `P1` `model-routing,#653`
- **来源 issue**：#653
...
#### [L1-8] 跨过程数据流求解器（worklist 不动点 + 格抽象） `P1` `sast,#596,#598`
- **来源 issue**：#598（合并 #608 状态机分析、#600 配置分析、#602 二进制分析、#601 密钥分析同属 MECE 漏洞挖掘体系；见合并说明）
...
#### [L1-9] 状态机分析（对象生命周期/反序列化协议/Feature 交互 FSM） `P1` `sast,#C,#608`
- **来源 issue**：#608
...
- **依赖/风险**：阻塞于 #598 求解器 + #590 typestate；CFG 异常边对 CFG 正确性极敏感（Swift defer/Rust ? 易画错）。

#### [L1-10] 配置文件漏洞分析（AndroidManifest/Info.plist/manifest.json/CI） `P1` `sast,#600`
- **来源 issue**：#600
...
- **依赖/风险**：纯规则匹配，成本可控；与 #601 密钥规则、#602 二进制解包复用。

#### [L1-11] LSP callHierarchy 补齐（递归调用链展开） `P1` `sast,ide,#710`
- **来源 issue**：#710
...
#### [L1-12] 密钥凭据泄漏分析生产化 `P2` `sast,#601`（疑似可关闭，见第三章）
- **来源 issue**：#601
...
#### [L1-13] 已编译产物分析（APK/IPA 外部工具编排） `P2` `sast,#602`
- **来源 issue**：#602
- **问题/动机**：移动端黑盒审计只有 APK/IPA，无源码，当前零能力。
- **可实施范围**：外部编排 apktool/jadx/otool 等解包 → 提取 manifest/strings/so → 喂给 #600 配置规则 + #601 密钥规则 + #588 污点（降级）。不自研反编译器。
- **验收口径**：APK 解包后 AndroidManifest 命中 #600 规则、strings 命中 #601 规则；聚合报告。
- **依赖/风险**：P2，依赖 #600/#601/#588 先落地；编排层成本可控。

#### [L1-14] 运行时插桩 + 运行时污点（MCP 外接） `P2` `sast,runtime,#651`
- **来源 issue**：#651
...
#### [L1-15] 覆盖率反馈 + 语法感知模糊测试 `P2` `sast,fuzzing,#652`
- **来源 issue**：#652
...
#### [L1-16] 假设→实证闭环（Hypothesis/Evidence/Verdict 一等公民） `P1` `harness,meta,#649`
- **来源 issue**：#649（与 #650 对抗验证同属 harness 元能力）
...
#### [L1-17] 多代理对抗验证（find vs refute） `P1` `harness,meta,#650`
- **来源 issue**：#650
...
#### [L1-18] 三个低成本工具壳（create_sub_session / record_artifact / 主会话 worktree） `P1` `tooling,#697`
- **来源 issue**：#697
...
#### [L1-19] Arena 交互式多模型同题对决 `P2` `benchmark,qw,#701`
- **来源 issue**：#701
...
#### [L2-1] IDE 上下文感知（visibleFiles/openTabs/光标位置） `P1` `ide,#70,#684`
- **来源 issue**：#684
...
- **依赖/风险**：需 ACP server 扩容（与 #686 协同）；不依赖特定 IDE。

#### [L2-2] Diff 逐行评论回灌（人→模型反馈通道） `P1` `ux,#68,#683`
- **来源 issue**：#683
...
#### [L2-3] Fleet 告警生产接线 + IM 渠道扩展 `P1` `fleet,#96,#687`
- **来源 issue**：#687（与 #97 跨端审批已落地区分开）
...
#### [L2-4] daemon WebSocket + ACP 扩容 + 多工作区注册表 `P1` `daemon,#93,#94,#686`
- **来源 issue**：#686
...
#### [L2-5] Provider OAuth + Bedrock/Vertex/Copilot 接入 `P1` `provider,#59,#61,#685`
- **来源 issue**：#685
...
#### [L2-6] 渐进式工具披露 select_tools 收尾（模型主动申请工具） `P1` `ux,prefix-cache,#708`
- **来源 issue**：#708（#725 已作为重复关闭）
...
#### [L2-7] BTW 侧边对话（独立消息栈临时旁路问答） `P2` `ux,#657`
- **来源 issue**：#657
...
#### [L2-8] headless 结构化输出（--json-schema 终态约束 + 合成终结工具） `P1` `headless,ux,#698`
- **来源 issue**：#698
...
#### [L3-1] 离线 mock 驱动 agent 行为评分基准（27 指标记分卡） `P1` `benchmark,#C8,#676`
- **来源 issue**：#676
...
- **依赖/风险**：与 #689 Edit Apply 基准、#629 compaction 断言互补；纯 Python+Rust 集成测试。

#### [L3-2] Edit Apply 一次成功率回归基准 `P1` `benchmark,#689`
- **来源 issue**：#689（关联 #676、#556）
...
- **依赖/风险**：依赖 #676 mock 基础设施或独立 harness；样本集需覆盖边界。

#### [L3-3] 公开记忆基准验证（LongMemEval）与能力提升追踪 `P1` `benchmark,memory,#777`
- **来源 issue**：#777
...
- **#601 密钥凭据泄漏分析** — 证据：`crates/secrets/src/scanner.rs` 已含 `scan_line`(L102)、`is_sensitive_content`(L253)、`redact_stream`(L308) 实现及单测（L465+）；调用点 `file.rs`/`edit.rs` 写入前拦截。SECURITY_CAPABILITY_PLAN.md 已将其列为「已完成前提」。建议复核是否还需补 git 历史扫描/熵分析，否则关闭。

- **#619 provider 故障转移/熔断** — 证据：`crates/tui/src/llm_client/circuit_breaker.rs`（纯状态机）+ `crates/tui/src/core/engine/circuit_breaker.rs`（引擎接线，`engine.rs:3104 mod circuit_breaker;`）+ `record_failure` 调用已存在并有单测。跨 provider 故障转移（`advance_fallback`）issue 评论已确认实现。**但** `impl_composer.rs`（advance_fallback 主体）grep 零命中 `CircuitBreaker` 引用，即熔断状态机已落地但「挂到 advance_fallback readiness 过滤」的收尾接线待确认。建议核实 advance_fallback 是否读取熔断状态——若已接线则关闭，若未接线则保留并收窄为「熔断收尾接线」（即 L1-6）。

- **#726 OpenTelemetry/OTLP 切片 A** — 证据：`crates/telemetry/` crate 已存在（`Cargo.toml`+`src/lib.rs`+`tests`），issue 评论确认 PR #772（`03c50f4`）已合并 feature-gated OTel 桥接。切片 B–D（GenAI span/Prometheus/endpoint 配置）未做。建议将 issue 收窄为「B–D 上层导出」（即 L0-8），或拆分后关闭切片 A 部分。

- **#716 记忆巩固与遗忘 切片 A** — 证据：`crates/memory/src/consolidation.rs` 已存在（`MemoryEntry{importance,last_accessed_at,access_count}` + `record_access`），PR #773（`a1f0320`）合并，46 passed。切片 B–E 未做。建议保留 issue 但更新状态，切片 A 标记已交付。

- **#714 混合检索 切片 A/B** — 证据：`crates/memory/src/codebase.rs` 已有 `RetrievalHit`(L57) + `score_breakdown`；PR #767/#771 已合并。切片 C/D/E（时近/重要性/四路融合）未做。建议保留并收窄为「切片 C–E」。

- **#732 用户建模 UserProfile 切片 A** — 证据：`crates/memory/src/user_profile.rs` 已有 `struct UserProfile`(L59)+`impl`(L88)，PR #770（`ce64497`）合并。切片 B–D 未做。建议保留并收窄为「切片 B–D」。

- **#653 统一次级模型 部分** — 证据：`crates/tui/src/model_routing/mod.rs:53` 已有 `enum SecondaryModel` + 解析逻辑 + 单测（L690+）。收敛进行中，但 seam_model/cheap_tier 三处是否真统一待确认。建议核实是否已统一，若已统一则关闭，否则保留。

## 四、合并说明

- **Epic #658 经验/训练闭环族**：#659（经验学习闭环）、#664（PTC 程序化工具调用）、#661（/learn 蒸馏 skill）、#660（Skill Curator）、#669（rollout 导出）、#672（Blueprint 导出）语义独立但同属「self-improvement / 训练数据生产线」主题。合并为 L1-1 一条需求项（主条目 #659），其余作为子能力在来源中并列标注，避免 6 条分散冲淡主线。PTC（#664）实为「脚本内 RPC 调工具」独立能力，若实施优先级高可单列。

- **MECE 漏洞挖掘体系**：#598（数据流求解器）、#608（状态机）、#600（配置分析）、#602（二进制分析）、#601（密钥分析）同属 `SECURITY_CAPABILITY_PLAN.md` 的 SAST 族，且 #600/#602 是 #598 求解器的上层应用、#601 已落地。合并为 L1-8~L1-12 系列（#598 作底座主条目，#600/#602/#601 各自独立需求项但共享依赖说明），#608 独立为 L1-9（C2/C3 专属）。

- **记忆体系**：#716（巩固遗忘）、#714（混合检索）、#732（UserProfile）、#679（模块合并）、#628（可观测）全部围绕 `crates/memory`，合并为 L1-2~L1-5 + L0-2 系列，明确切片 A 已交付状态，避免重复派工。

- **评测体系**：#676（agent 行为基准）、#689（Edit Apply 基准）、#777（LongMemEval）合并为 L3 三件套，分别覆盖「行为记分卡 / 编辑成功率 / 记忆质量」三个正交维度。

- **Provider 可靠性**：#619 故障转移/熔断经核验故障转移已实现、熔断部分落地，与相关 #419 类不在列表；保留为 L1-6 单条（P0，因影响主干长任务可用性）。

- **QUEUE/长程任务族**（#654/#700/#631/#665/#693/#724）经核查**不在 43 个 OPEN 列表中**（已关闭或并入他处），其能力地图在 `QUEUE_CAPABILITY_PLAN.md` 已闭环描述，本梳理不重复列入，仅作为背景依赖（如 #653 阻塞 T-Q5）。

- **重复关闭项**：#725（select_tools 重复）已关闭指向 #708；#662（MemoryBackend 重复）已关闭拆出 #732；#623（记忆巩固重复）已关闭并入 #716。均不重复计。

---

**共梳理 43 个 OPEN issue，归并为 37 条需求项（L0:8 / L1:18 / L2:8 / L3:3）。**
