...
| 指令 | 说明 | 示例 |
|------|------|------|
| `/compact` | 压缩上下文，可附带压缩指令 | `/compact 只保留 API 改动` |
| `/context` | 查看上下文用量分布 | `/context` |
| `/purge` | 由 Agent 外科式清理上下文 | `/purge` |
| `/tokens` | 查看 token 统计 | `/tokens` |

`/compact` 后面可以直接跟自然语言，指定这次摘要要重点保留什么：

```bash
/compact 重点保留认证重构的决策，测试调试过程可以省略
```

如果每次压缩都想强调同样的重点，把它写进项目根目录 `AGENTS.md` 的
`# Compact Instructions` 小节，手动压缩与自动压缩都会遵循：

```markdown
# Compact Instructions

保留数据库迁移相关的改动与报错原文，探索性的调试过程可以省略。
```

命令行上临时给出的指令，优先级高于 `AGENTS.md` 中的常驻配置。两种方式都是
**追加**约束：文件路径、报错、待办等连续性信息始终会被保留，不会因为自定义
指令而丢失。
