...
Workrooms are mimo-tui's chat-native abstraction for durable, addressable
...
1. **Creation.** A future workroom is created when a thread is started with a
   ...
   is stable and can be shared as a `mimo-tui://workroom/...` link.
...
3. **Link resolution.** When a `mimo-tui://workroom/...` link appears in
...
Persisted workroom state should live alongside existing mimo-tui state:
...
|---|---|
| `mimo-tui-protocol` | Types: `Workroom`, `WorkroomId`, `WorkroomThread`, `WorkroomEvent`, `WorkroomLink`, `ExternalThreadRef`, `AgentAttribution` |
| `mimo-tui-app-server` | Future endpoints: `GET /workrooms`, `GET /workroom/:id/threads`, `GET /workroom/resolve` |
| `mimo-tui` | Future model-facing link resolution and optional sidebar inbox |
| `mimo-tui-state` | Future: persistent workroom store (Phase 2) |
