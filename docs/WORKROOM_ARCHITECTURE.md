...
Workrooms are mimofan's chat-native abstraction for durable, addressable
...
1. **Creation.** A future workroom is created when a thread is started with a
   ...
   is stable and can be shared as a `mimofan://workroom/...` link.
...
3. **Link resolution.** When a `mimofan://workroom/...` link appears in
...
Persisted workroom state should live alongside existing mimofan state:
...
|---|---|
| `mimofan-protocol` | Types: `Workroom`, `WorkroomId`, `WorkroomThread`, `WorkroomEvent`, `WorkroomLink`, `ExternalThreadRef`, `AgentAttribution` |
| `mimofan-app-server` | Future endpoints: `GET /workrooms`, `GET /workroom/:id/threads`, `GET /workroom/resolve` |
| `mimofan` | Future model-facing link resolution and optional sidebar inbox |
| `mimofan-state` | Future: persistent workroom store (Phase 2) |
