...
- `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, and `mimo-v2.5` are treated as
  1,000,000-token Xiaomi MiMo V2.5 chat models in mimofan metadata.
- `mimo-v2-omni` remains a 256K-window V2-series model; mimofan does not use
  it as the current `xiaomi-mimo` Omni shorthand.
- Token Plan usage is credit/quota based and is not interoperable with
  pay-as-you-go account balance. mimofan therefore leaves direct
...
assert mimofan's bundled metadata and provider behavior; a future docs-refresh
