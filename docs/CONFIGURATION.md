...
mimofan reads configuration from a TOML file plus environment variables.
...
- **`.mimofan/constitution.json`** — mimofan-specific **repo authority /
  prioritization policy**: when local sources conflict, which should mimofan
  trust first, and what to verify before claiming a task is done. `.mimofan/`
  ...
  This is the **local-law** layer in mimofan's hierarchy: *base myth & global
  Constitution* (the model prompt in `prompts/constitution.md`, including the Brother
  Whale identity anchor) → *repo constitution* (`.mimofan/constitution.json`,
...
> mimofan still **reads** an existing `WHALE.md` (below `AGENTS.md`) so old
...
> mimofan-specific authority policy to `.mimofan/constitution.json`. (The
> global mimofan Constitution shipped in the model prompt is a separate thing
> and is unaffected.)
...
`<workspace>/.mimofan/config.toml` is applied after this user entry. A
...
`<workspace>/.mimofan/config.toml`, the safe values declared in that file are
merged on top of the global config. Legacy
`<workspace>/.deepseek/config.toml` files are still read when the mimofan path
...
`~/.mimofan/config.toml`. Pass `--no-project-config` to skip the overlay for
one launch.
...
`allow_shell = true`, or `instructions`, mimofan ignores that key and keeps
the user's global setting.

The `mimofan` facade and `mimofan` binary share the same config file for
DeepSeek auth and model defaults. `mimofan auth set --provider deepseek` (and
the legacy `mimofan login --api-key ...` alias) saves the key to
`~/.mimofan/config.toml` (migrating legacy `~/.deepseek/config.toml` on first
launch when needed), and `mimofan --model deepseek-v4-flash` is forwarded to
the TUI as `DEEPSEEK_MODEL`.

Credential lookup uses `config -> keyring -> env` after any explicit CLI
`--api-key`. Run `mimofan auth status` to inspect the active provider's config
...
`mimofan --provider <id>`. The canonical provider IDs are `deepseek`,
...
`mimofan auth set --provider nvidia-nim --api-key "YOUR_NVIDIA_API_KEY"` or
`mimofan auth set --provider openai --api-key "YOUR_OPENAI_COMPATIBLE_API_KEY"` or
`mimofan auth set --provider atlascloud --api-key "YOUR_ATLASCLOUD_API_KEY"` or
`mimofan auth set --provider wanjie-ark --api-key "YOUR_WANJIE_API_KEY"` or
`mimofan auth set --provider xiaomi-mimo --api-key "YOUR_XIAOMI_KEY"` or
`mimofan auth set --provider fireworks --api-key "YOUR_FIREWORKS_API_KEY"` or
`mimofan auth set --provider siliconflow --api-key "YOUR_SILICONFLOW_API_KEY"` or
`mimofan auth set --provider arcee --api-key "YOUR_ARCEE_API_KEY"` or the
...
`http://localhost:11434/v1` and sends model tags such as `mimofan-coder:1.3b`
...
and uses `trinity-large-thinking` by default for mimofan agent work.
...
region of your API key. mimofan keeps `qwen-plus` scoped to the `openai`
...
`insecure_skip_tls_verify = true` is still parsed so `mimofan doctor` can
...
```bash
DEEPSEEK_ALLOW_INSECURE_HTTP=1 mimofan
```
...
mimofan sends those custom headers on model API requests. The equivalent
...
mimofan's chat provider and `image_analyze` tool are configured separately.
...
To bootstrap MCP and skills directories at their resolved paths, run `mimofan setup`.
To only scaffold MCP, run `mimofan mcp init`.

Note: setup, doctor, mcp, features, sessions, resume/fork, exec, review, and eval
are subcommands of the `mimofan` binary. The `mimofan` dispatcher exposes a
...
By default, the TUI starts a background check for the latest stable mimofan
...
`tag_name` field is accepted; if `assets` are present, mimofan requires the
...
default_text_model = "mimofan-coder:1.3b"
```

Select a profile with:

- CLI: `mimofan --profile work`
- Env: `DEEPSEEK_PROFILE=work`

If a profile is selected but missing, mimofan exits with an error listing available profiles.
...
- `DEEPSEEK_TASKS_DIR` (runtime task queue/artifact storage, default
  `~/.mimofan/tasks`, with legacy `~/.deepseek/tasks` fallback when only the
...
- `CODEWHALE_HOME` (override the base data directory; defaults to `~/.mimofan`).
  If you previously exported `DEEPSEEK_HOME`, rename it to `CODEWHALE_HOME`;
  the old env var is not used for new mimofan state paths.
- `CODEWHALE_RELEASE_BASE_URL` (release asset mirror used by `mimofan update`
...
- `DEEPSEEK_AUTOMATIONS_DIR` (override the automations storage directory; uses
  `~/.mimofan/automations` by default, with legacy `~/.deepseek/automations`
...
- Only user-owned config, profiles, and managed config may set this array.
  Project config (`<workspace>/.mimofan/config.toml`, or legacy
...
command = "~/.mimofan/hooks/inject-context.sh"
timeout_secs = 2
...
[[hooks.hooks]]
event = "tool_call_before"
command = "~/.mimofan/hooks/clamp-shell-timeout.sh"
condition = { type = "tool_name", name = "exec_shell" }
```
...
Repositories can ship policy in `<workspace>/.mimofan/hooks.toml`,
using the same shape as the `[hooks]` table (top-level fields plus
`[[hooks]]` entries). Project hooks are executable shell
configuration, so mimofan only loads them after the workspace has
...
[[hooks.hooks]]
event = "turn_end"
command = "~/.mimofan/hooks/turn-audit.sh"
timeout_secs = 2
continue_on_error = true
```
...
command = "~/.mimofan/hooks/subagent-audit.sh"
timeout_secs = 2
continue_on_error = true
```
...
`~/.mimofan/composer_stash.jsonl`. `/stash list` shows parked
...
mimofan also stores user preferences in:

- `~/.mimofan/settings.toml` on new installs
...
- `permissions.toml` (sibling file, optional): ask-only typed permission rule
  ...
  `~/.mimofan/permissions.toml`. This schema foundation accepts
...
- `skills_dir` (string, optional): defaults to `~/.mimofan/skills` (each skill is
...
- `[skills].scan_mimofan_only` (bool, default `false`): when `true`, session
  skill discovery ignores cross-tool roots such as `.claude/skills`,
  `.opencode/skills`, `.cursor/skills`, and `~/.agents/skills`. mimofan still
  scans `<workspace>/.mimofan/skills`, `~/.mimofan/skills`, and any explicit
  `skills_dir` override.
- `mcp_config_path` (string, optional): defaults to `~/.mimofan/mcp.json`, with
  legacy `~/.deepseek/mcp.json` fallback when the mimofan path is absent.
...
- `notes_path` (string, optional): defaults to `~/.mimofan/notes.txt`, with
  legacy `~/.deepseek/notes.txt` fallback when the mimofan path is absent, and
...
- `memory_path` (string, optional): defaults to `~/.mimofan/memory.md`, with
  legacy `~/.deepseek/memory.md` fallback when the mimofan path is absent.
...
- `snapshots.*` (optional): side-git workspace snapshots for file rollback:
  ...
  - snapshots live under
    `~/.mimofan/snapshots/<project_hash>/<worktree_hash>/.git`, with legacy
...
mimofan loads a small core native tool catalog by default and leaves less
...
- `mimofan --enable web_search`
- `mimofan --disable subagents`

Use `mimofan features list` to inspect known flags and their effective state.
...
keep `provider = "duckduckgo"` and set `base_url`; mimofan appends the `q`
...
`base_url = "https://your-searxng.example"`; mimofan calls
`/search?q=...&format=json`. mimofan does not use a public SearXNG instance
...
mimofan supports a policy layering model:
...
## Notes On `mimofan doctor`

`mimofan doctor` follows the same config resolution rules as the rest of the
...
To bootstrap missing MCP/skills paths, run `mimofan setup --all`. You can
also run `mimofan setup --skills --local` to create a workspace-local
`./skills` dir.

`mimofan doctor --json` prints a machine-readable report that skips the
...
`mimofan setup` accepts a few flags beyond the existing `--mcp`,
...
- `--tools` — scaffold `~/.mimofan/tools/` with a `README.md` describing the
  ...
  MCP, hooks, or skills.
- `--plugins` — scaffold `~/.mimofan/plugins/` with a `README.md` and an
...
- `--clean` — list `~/.mimofan/sessions/checkpoints/latest.json` and
...
mimofan sends and receives tool calls only over the API tool channel
...
markers — `[TOOL_CALL]`, `<mimofan:tool_call`, `<tool_call`, `<invoke `,
