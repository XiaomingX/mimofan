...
runtime records that mimofan already owns: thread metadata, turn status, turn
...
```text
mimofan receipt export --thread <thread_id> --turn <turn_id> --format json
...
`mimofan review --write-receipt` writes a local JSON receipt for the reviewed
diff under the mimofan state directory (`review-receipts/`) unless
...
`mimofan review --write-receipt` after changing the diff; reviewers should
compare the `diff_fingerprint` before reusing a receipt in a PR handoff.

`mimofan review --check-receipt` is the local pre-push gate. It does not call
...
```json
{
  "schema_id": "mimofan.conformance-receipt/v0",
  "thread": {
    "id": "thr_...",
