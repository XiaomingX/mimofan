...
This guide is for your first hour with mimofan. It explains the main
workflow, the important safety controls, and where to go next when you need a
complete reference.

mimofan has deeper reference documents for installation, configuration,
...
## 1. Welcome to mimofan

mimofan is a terminal coding agent. You run it from a workspace, give it a
task, and it can use structured tools to inspect files, run commands, edit
code, and report back with evidence.

The important difference from a normal chat model is that mimofan is built
around a harness:
...
You can use mimofan for small questions:
...
For a new repository, start conservatively. Ask mimofan to explore and plan
...
Install mimofan with the path that fits your machine. Each supported install
path provides both the `mimofan` dispatcher and the `mimofan` runtime.
...
Launch mimofan from the repository or directory you want it to work in:

```bash
mimofan
```

On first launch, mimofan needs an API key for the active provider. DeepSeek is
...
New mimofan config is stored under `~/.mimofan/config.toml`. Legacy
...
```text
Investigate why `mimofan doctor` reports the wrong provider. Do not edit
files yet. Return the likely cause, evidence, and a proposed patch plan.
```

mimofan works best when you let investigation and implementation happen in
...
The transcript is the audit trail. When mimofan reads files, runs commands,
...
mimofan has three visible TUI modes:
...
Agent mode is the default for most contribution work. It lets mimofan read,
...
change mimofan state directly instead of asking the model in natural
language.
...
Use `/model auto` when you want mimofan to choose the model and thinking
...
mimofan tools are structured actions. Instead of only producing prose, the
...
The workspace boundary matters. mimofan is expected to work in the directory
...
that teaches mimofan how to perform a recurring workflow, use a tool family,
...
### Is mimofan only for DeepSeek?

DeepSeek is the default and first-class route, but mimofan also supports
other hosted and local OpenAI-compatible providers. Use `/provider` or
`mimofan --provider <id>` to choose a provider. Keep the provider registry
...
### Why does mimofan ask before running commands?
...
Inside mimofan, ask the agent to inspect the file and run it with
...
New mimofan config uses `~/.mimofan/config.toml`. Legacy
...
profile, and compact long sessions. For larger tasks, ask mimofan to plan
before implementing so you do not spend tokens on the wrong path.

### How do I continue previous work?

mimofan saves sessions. Use the session picker or resume/continue CLI paths
...
from every workspace, or run `mimofan sessions` to list all saved sessions
...
problem is operational, run `mimofan doctor` and inspect the reported config
and provider state.
...
### Can mimofan edit files outside the current repository?
