...
This guide is for your first hour with mimo-tui. It explains the main
workflow, the important safety controls, and where to go next when you need a
complete reference.

mimo-tui has deeper reference documents for installation, configuration,
...
## 1. Welcome to mimo-tui

mimo-tui is a terminal coding agent. You run it from a workspace, give it a
task, and it can use structured tools to inspect files, run commands, edit
code, and report back with evidence.

The important difference from a normal chat model is that mimo-tui is built
around a harness:
...
You can use mimo-tui for small questions:
...
For a new repository, start conservatively. Ask mimo-tui to explore and plan
...
Install mimo-tui with the path that fits your machine. Each supported install
path provides both the `mimo-tui` dispatcher and the `mimo-tui` runtime.
...
Launch mimo-tui from the repository or directory you want it to work in:

```bash
mimo-tui
```

On first launch, mimo-tui needs an API key for the active provider. DeepSeek is
...
New mimo-tui config is stored under `~/.mimo-tui/config.toml`. Legacy
...
```text
Investigate why `mimo-tui doctor` reports the wrong provider. Do not edit
files yet. Return the likely cause, evidence, and a proposed patch plan.
```

mimo-tui works best when you let investigation and implementation happen in
...
The transcript is the audit trail. When mimo-tui reads files, runs commands,
...
mimo-tui has three visible TUI modes:
...
Agent mode is the default for most contribution work. It lets mimo-tui read,
...
change mimo-tui state directly instead of asking the model in natural
language.
...
Use `/model auto` when you want mimo-tui to choose the model and thinking
...
mimo-tui tools are structured actions. Instead of only producing prose, the
...
The workspace boundary matters. mimo-tui is expected to work in the directory
...
that teaches mimo-tui how to perform a recurring workflow, use a tool family,
...
### Is mimo-tui only for DeepSeek?

DeepSeek is the default and first-class route, but mimo-tui also supports
other hosted and local OpenAI-compatible providers. Use `/provider` or
`mimo-tui --provider <id>` to choose a provider. Keep the provider registry
...
### Why does mimo-tui ask before running commands?
...
Inside mimo-tui, ask the agent to inspect the file and run it with
...
New mimo-tui config uses `~/.mimo-tui/config.toml`. Legacy
...
profile, and compact long sessions. For larger tasks, ask mimo-tui to plan
before implementing so you do not spend tokens on the wrong path.

### How do I continue previous work?

mimo-tui saves sessions. Use the session picker or resume/continue CLI paths
...
from every workspace, or run `mimo-tui sessions` to list all saved sessions
...
problem is operational, run `mimo-tui doctor` and inspect the reported config
and provider state.
...
### Can mimo-tui edit files outside the current repository?
