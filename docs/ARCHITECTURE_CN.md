...
mimo-tui 是一个 **AI 编码助手**，你给它一句话（比如"帮我写个 REST API"），它调用大模型思考，然后用工具帮你改代码、跑命令、查文件，直到把活干完。
...
- **用户交互层**：你看到的界面。TUI 是终端里的可视化界面（类似 vim），CLI 是命令行工具，HTTP 服务器让其他程序可以通过网络调用 mimo-tui。
...
mimo-tui 的提示词采用**分层治理**架构，优先级从高到低：
