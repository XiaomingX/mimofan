...
> 本文档面向开发者，用大白话讲清楚 CodeWhale 的架构设计、模块职责、核心入口和扩展方式。

---

## 一句话说清楚

CodeWhale 是一个 **AI 编码助手**，你给它一句话（比如"帮我写个 REST API"），它调用大模型思考，然后用工具帮你改代码、跑命令、查文件，直到把活干完。
...
- **用户交互层**：你看到的界面。TUI 是终端里的可视化界面（类似 vim），CLI 是命令行工具，HTTP 服务器让其他程序可以通过网络调用 CodeWhale。
...
```
                   ...
                   /      |      \        /     |     \
                  /       |       \      /      |      \
        app-server    release   secrets  core  agent  config
         /  |  \        |         |       |     |      / \
        /   |   \       |         |       |     |     /   \
...
```rust
// 创建运行时
let runtime = Runtime::new(config, model_registry, ...).await?;

// 处理用户输入
let response = runtime.handle_prompt(thread_id, user_message).await?;

// 调用工具
let output = runtime.invoke_tool(tool_call).await?;

// 管理后台任务
runtime.enqueue_job("my-job", job_payload).await?;
```
...
```rust
let store = StateStore::open("~/.codewhale/state.db")?;
...
# 运行测试
cargo test -p codewhale-config        # 配置测试
cargo test -p codewhale-protocol      # 协议测试
cargo test -p codewhale-tui --locked  # TUI 测试
...
- `config_command_allow_shell_*` 在 `~/.codewhale/settings.toml` 设置 `default_mode = "yolo"` 时失败（测试不隔离）
