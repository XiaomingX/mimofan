# The mimofan Agent Runtime — one durable substrate, familiar launchers
...
- **`mimofan exec`** is the headless front door: usable by anyone at any time
...
- A **fleet worker** *is* a `mimofan exec` run that the fleet launches and
  tracks durably — locally as a subprocess, or remotely as
  `ssh host … mimofan exec …`. The fleet does not re-implement execution; it
...
then the cutover is incomplete. Treat that as a mimofan runtime gap, not as
...
`mimofan exec`. The heavy construction then lives in another process entirely,
...
simulated taxonomy. `mimofan exec --output-format stream-json` emits
...
mimofan should converge with Claude Code on **shape**, not on branding:
...
- **Keep distinct**: mimofan branding and first-class DeepSeek/GLM/MiniMax/
  multi-provider support; the local-first **Agent Fleet** (durable, SSH-capable
  orchestration) as mimofan's own layer above the shared runtime; WhaleFlow as
