---
name: gh-find-prs
description: "Survey open mimo-tui PRs and triage each for mergeability and disposition against the real landing branch."
---
...
1. **Inventory the queue.** One call, structured:
   ```
   gh pr list --repo XiaomingX/mimo-tui --state open \
...
3. **Read each candidate from code, not title.** For every non-trivial PR:
   ...
   gh pr diff <N> --repo XiaomingX/mimo-tui
   ```
...
4. **Decode check failures — distinguish trivial from real.** In `statusCheckRollup`, find each `conclusion: FAILURE` and read its job. mimo-tui's CI jobs are `Lint`, `Test (ubuntu-latest|macos-latest|windows-latest)`, `Version drift`, `gate` (Contribution gate), `npm wrapper smoke`, `Mobile runtime smoke`, `Documentation`, `GitGuardian Security Checks`.
