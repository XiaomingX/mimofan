...
description: "Close resolved mimo-tui issues only after verifying the landed commit/behavior, with a positive crediting comment; never from title alone."
---
...
Repo: `XiaomingX/mimo-tui`. CLI: `gh`.
...
5. **Close with that comment in one step** (only with maintainer approval where
   policy requires it):
   ```bash
   gh issue close N --repo XiaomingX/mimo-tui -r completed \
...
keep it open:
```bash
gh issue comment N --repo XiaomingX/mimo-tui \
