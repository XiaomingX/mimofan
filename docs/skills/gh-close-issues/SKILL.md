...
description: "Close resolved mimofan issues only after verifying the landed commit/behavior, with a positive crediting comment; never from title alone."
---
...
5. **Close with that comment in one step** (only with maintainer approval where
   policy requires it):
   ```bash
   gh issue close N --repo XiaomingX/mimofan -r completed \
...
keep it open:
```bash
gh issue comment N --repo XiaomingX/mimofan \
