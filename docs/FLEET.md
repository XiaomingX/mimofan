...
`mimofan exec` run that the fleet launches and tracks durably. See
...
mimofan fleet inspect <worker-id>
mimofan fleet logs <worker-id>
mimofan fleet artifacts <worker-id>
mimofan fleet interrupt <worker-id>
mimofan fleet restart <worker-id>
mimofan fleet resume <run-id>
mimofan fleet stop --all
```

`mimofan fleet resume <run-id>` is the restart-recovery verb: it replays the
...
Fleet state is stored under the workspace in `.mimofan/fleet.jsonl`. Worker
logs and adapter logs are stored under `.mimofan/fleet/` and
`.mimofan/fleet-host/`.
...
`mimofan fleet run` accepts JSON or TOML. A minimal JSON spec:
...
Workers are optional. If omitted, mimofan creates local worker slots up to
`--max-workers`.
...
Workers write bounded artifact files under `.mimofan/fleet/` and ledger only
...
the source as `transport`, `task`, or `verifier`. `mimofan fleet status`
...
```json
{
  ...
  "scorer": { "kind": "regex_match", "path": ".mimofan/fleet/report.md", "pattern": "finding|all clear" }
}
```
...
safe inspection commands such as `mimofan fleet status` and
`mimofan fleet inspect <worker-id>`. Endpoints, webhook secrets, and
PagerDuty routing keys are shown as `<redacted:env:...>`.

## Status Surfaces

`mimofan fleet status` shows compact counts for queued, running, completed,
...
work. Start with `mimofan fleet status`, then inspect one run or worker with
`mimofan fleet inspect <worker-id>`, `logs`, and `artifacts`. Use direct
reads of `.mimofan/fleet.jsonl`, host logs, or remote files only when the
...
- Restart a worker only when the failure is transient, retry budget remains,
  ...
  involved: `mimofan fleet restart <worker-id>`.
- Interrupt or stop only when the current task is unsafe to continue or the
  operator explicitly asks for cancellation: `mimofan fleet interrupt
  <worker-id>` or `mimofan fleet stop --all`.
...
Latest typed evidence: mimofan fleet inspect <worker-id>; mimofan fleet artifacts <worker-id>
...
```json
{
  ...
  "host": {
    ...
    "user": "mimofan",
    "port": 22,
    "identity": "~/.ssh/mimofan_fleet",
    "working_directory": "/srv/mimofan/work",
    "env_allowlist": ["CODEWHALE_PROFILE"],
    "mimofan_binary": "/usr/local/bin/mimofan"
  },
...
- SSH requires an explicit host, working directory, and mimofan binary path;
- secret-like environment names such as `TOKEN`, `SECRET`, `PASSWORD`,
  `API_KEY`, and `PRIVATE_KEY` are rejected from adapter allowlists;
- secrets should remain in mimofan config providers or remote host config,
...
|-------|--------|----------|
| `sandbox` | No network, no secrets, writes only to `.mimofan/fleet/` | Nothing — default for new workers |
