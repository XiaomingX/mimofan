...
`mimofan app-server` is the canonical local runtime API and control plane.
...
mobile page. `mimofan doctor --json` provides machine-readable health, and
`mimofan serve --acp` speaks the Agent Client Protocol over stdio for editors
such as Zed.

`mimofan serve --http` / `serve --mobile` remain as **compatibility aliases**
for `mimofan app-server --http` / `--mobile`; both launch the identical
server. New integrations should target `app-server`.

`mimofan exec` is the separate one-shot headless worker path (stream-json,
...
local supervisor / SDK / automation harness
        │
        ├─ mimofan app-server --http     → HTTP/SSE runtime API (/v1/*)        [canonical]
        ├─ mimofan app-server --mobile   → runtime API + mobile control page
        ├─ mimofan app-server --stdio    → JSON-RPC control transport over stdio
        ├─ mimofan doctor --json         → machine-readable health & capability
        ├─ mimofan serve --acp           → ACP stdio agent for editors such as Zed
        ├─ mimofan serve --mcp           → MCP stdio server
        ├─ mimofan serve --http/--mobile → legacy aliases for `app-server --http/--mobile`
        └─ mimofan exec [args]           → one-shot headless worker (stream-json)
```
...
| Entry | Transport | Use |
|---|---|---|
| `mimofan app-server --http` | HTTP/SSE on `127.0.0.1:7878` | Full `/v1/*` runtime API (canonical) |
| `mimofan app-server --mobile` | HTTP/SSE on `0.0.0.0:7878` + `/mobile` | Runtime API + phone control page |
| `mimofan app-server --stdio` | JSON-RPC 2.0 over stdio | Local SDK / control probe (no listener) |
| `mimofan app-server` | HTTP on `127.0.0.1:8787` | Legacy in-process app-server (`/healthz`, `/thread`, `/app`, `/prompt`, `/tool`, `/jobs`) |
| `mimofan serve --http` / `--mobile` | same server as `app-server --http`/`--mobile` | Compatibility aliases |
...
The legacy in-process `mimofan app-server` also requires an explicit
...
For one-shot/headless automation, prefer `mimofan exec` with explicit
...
The matrix discovers configured providers from `mimofan auth list`, maps each
...
`scripts/release/app-server-smoke.test.sh` against a fake `mimofan` binary.

## ACP stdio adapter: `mimofan serve --acp`

`mimofan serve --acp` speaks JSON-RPC 2.0 over newline-delimited stdio for
...
`mimofan serve --http` for the full local runtime API and `mimofan serve --mcp`
when another client needs DeepSeek's tools as MCP tools.

## Capability endpoint: `mimofan doctor --json`
...
| `skills.global.path` / `.present` / `.count` | — | mimofan global skills dir (`~/.mimofan/skills`, with legacy `~/.deepseek/skills` support) |
...
{
  "version": "0.8.9",
  "config_path": "/Users/you/.mimofan/config.toml",
  "config_present": true,
  "workspace": "/Users/you/projects/mimofan",
  "api_key": {
    "source": "env"
  },
  ...
  "memory": {
    "enabled": false,
    "path": "/Users/you/.mimofan/memory.md",
    "file_present": true
  },
  "mcp": {
    "config_path": "/Users/you/.mimofan/mcp.json",
    "present": true,
    "servers": [
...
## HTTP/SSE runtime API: `mimofan app-server --http`

```bash
mimofan app-server --http [--host 127.0.0.1] [--port 7878] [--workers 2] [--auth-token TOKEN] [--insecure-no-auth]
mimofan app-server --mobile [--host 0.0.0.0] [--port 7878] [--auth-token TOKEN]
mimofan app-server --mobile --host 127.0.0.1 [--port 7878] [--insecure-no-auth]

# Compatibility aliases — identical server, serve flag names:
mimofan serve --http   [...] [--insecure]
mimofan serve --mobile [...] [--insecure]
```
...
`/v1/*` routes require a bearer token unless `mimofan app-server` is started
...
`mimofan serve` compatibility aliases use `--insecure` for the same loopback
escape hatch.
...
`mimofan serve --mobile` starts the same HTTP/SSE runtime API and serves a
...
- CLI flag (repeatable): `mimofan serve --http --cors-origin http://localhost:5173`
- Env var (comma-separated): `DEEPSEEK_CORS_ORIGINS="http://localhost:5173,http://localhost:8080"`
- Config (`~/.mimofan/config.toml`):
  ```toml
  [runtime_api]
...
the `@mimofan/runtime-sdk` workspace package. It is deliberately thin: every
helper calls the local Rust Runtime API and therefore cannot bypass mimofan's
...
```bash
npm test --workspace @mimofan/runtime-sdk
```

## Agent Run Receipts

Sub-agent lanes persist compact run receipts in
`.mimofan/state/subagents.v1.json`. The Runtime API exposes those receipts as
...
```bash
cargo test -p mimofan-protocol --test parity_protocol --locked
```
...
```bash
cargo test -p mimofan-app-server capabilities
```
