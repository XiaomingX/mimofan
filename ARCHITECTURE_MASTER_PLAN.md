...
| 问题 | 现状 | 影响 |
|------|------|------|
| 文件过大 | main.rs 9235 行，config.rs 5171 行 | 难以维护 |
| 职责混杂 | UI 渲染、业务逻辑、配置管理混在一起 | 修改困难 |
| 测试困难 | 340,148 行代码，测试覆盖率低 | 回归风险 |
...
### Phase 5: TUI crate 拆分 [ ]

**目标**：降低 TUI crate 复杂度，提高可维护性

- [ ] 从 `main.rs` 抽取初始化逻辑
  - 当前 9235 行，目标 < 2000 行
- [ ] 从 `config.rs` 抽取配置验证逻辑
  - 当前 5171 行，目标 < 1000 行
- [ ] 从 `client.rs` 抽取 HTTP 客户端逻辑
  - 当前 4876 行，目标 < 1000 行
- [ ] 统一 UI 渲染和业务逻辑分离
  - 创建 `ui/` 目录，按功能拆分
...
| TUI 拆分引入 UI 回归 | 中 | 高 | 保留原有测试，逐步拆分 |
...
- Phase 2-3 已完成，可立即继续
- Phase 4-6 可并行，但建议先完成 Phase 4（并发安全）
...
3. **替换 anyhow 为 thiserror**：二进制 crate 使用 anyhow 是合理选择
4. **完全消除 unwrap**：测试代码中的 unwrap 是可接受的
...
2. **硬编码残留**：配置路径和环境变量散落（已解决）
3. **错误处理不一致**：unwrap 泛滥（已解决）
4. **TUI crate 过于庞大**：需要拆分
5. **并发安全需加固**：锁粒度和 clone 优化

通过 6 个 Phase 的渐进式重构，可以在不影响用户体验的前提下，显著降低维护成本和稳定性风险。
