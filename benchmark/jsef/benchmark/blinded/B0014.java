 ...
 *
 * JSEF-Benchmark L5 — JWT 多环节绕过链（BX 对照，CWE-347）
 *
 * 同链安全对照：bx 侧 4 环节（弱算法 / 密钥可控 / 过期缺陷 / role 篡改）全未加固，
 * 串联形成绕过。本 BX 侧对每一环节都加固，任一环节即断链：
 ...
 * 4 环节全加固 → 攻击者无法篡改 role，授权 sink 不可达 → 判 BX。
 *
 * 难点/区分点：与 bx 侧同构的多环节链，仅每一环的加固方式不同；用于检验工具能否
 * 逐环判断"是否加固"从而区分 Bx/BX，而非因看到同一类 JWT 方法就一律报漏洞。
 *
 * CWE-347 (Improper Verification of Cryptographic Signature)。判 BX。
 */
public class JwtChainBypassBy {
    ...
    private String grantAccess(String role, String token) {
        System.out.println("[authz-by] role=" + role + " token=" + token);
        return "granted:" + role;
    }

    public static void main(String[] args) {
        new JwtChainBypassBy().authorize("demo.token", "none", "attacker-key", null);
    }
}
