 ...
 *
 * JSEF-Benchmark L4 — 跨集合/异步传播长链（BX 对照，CWE-89）
 *
 * 同链安全对照：bx 侧不可信输入经 CompletableFuture 三级异步链传播后拼入 SQL 串
 * （jdbcTemplate.queryForList）。本 BX 侧同样经历多级异步链（supplyAsync -> thenApply
 * -> thenCompose -> thenApply），但**末端用参数化查询**：不可信值作为绑定参数传入
 * PreparedStatement，绝不拼入 SQL 语句本身。
 *
 * 难点/区分点：
 *   - 与 bx 侧**同样的多级异步传播结构**，仅末端 sink 处理方式不同（拼接 vs 参数化）。
 *   - 用于检验工具能否识别"末端参数化/PreparedStatement"这一净化点，从而对同类异步链
 *     正确判 BX，而非因看到 CompletableFuture 链 + SQL 就一律报注入。
 *
 * CWE-89 (SQL Injection)。判 BX：不可信值进入绑定参数，非拼接。
 */
public class AsyncChainSqlBy {
    ...
    public static void main(String[] args) throws Exception {
        new AsyncChainSqlBy().run("1' OR '1'='1"); // 仅 localhost 演示语义，参数化后无注入
    }
}
