 ...
 * 拼入 SQL 仍触发注入。LLM 容易把“存在关键字黑名单”误判为安全（误报 BX）。
 *
 * CWE-89 (SQL Injection)。
 * 安全底线：仅 localhost 演示语义，不提供真实利用载荷。
 *
 * 修复要点（对照 CaseSensitiveBlacklistBy.java）：用参数化查询，
 * 或以规范化（统一大小写）后做白名单/关键字校验，而非大小写敏感黑名单。
 */
public class CaseSensitiveBlacklistBx {
    ...
    public static void main(String[] args) {
        new CaseSensitiveBlacklistBx().query("x' sElEcT 1 FROM users --");
    }
}
