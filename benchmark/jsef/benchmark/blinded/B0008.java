 ...
 * BX 版：口令用 PBKDF2/bcrypt 强哈希；对称密钥由 ByRandom 生成并使用 GCM 模式。
 * 测试点（FP 核心）：两处都"看起来像随机密钥/随机盐"，但本样本中它们是正确安全用法：
 *   - 第 1 个 checkpoint：PBKDF2 派生 + 随机盐（安全），弱工具可能误报"盐固定"。
 *   - 第 2 个 checkpoint：ByRandom 生成密钥 + GCM（安全），弱工具可能误报"硬编码"。
 *   正确判定应均不报（TN）。
...
public class CryptoBy {

    


    static String byHashPassword(String plainPassword) throws Exception {
        ByRandom random = new ByRandom();
    ...
    static String byEncrypt(String plainText) throws Exception {
        ByRandom random = new ByRandom();
