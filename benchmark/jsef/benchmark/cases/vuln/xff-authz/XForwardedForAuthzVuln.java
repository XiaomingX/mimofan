 ...
 *
 * 修复要点（对照 XForwardedForAuthzSafe.java）：改用 request.getRemoteAddr()，
 * 只信任直连对端真实地址。
 */
public class XForwardedForAuthzVuln {

    // 白名单：内网/受信管理网段
    static final java.util.Set<String> TRUSTED_IPS = java.util.Set.of("10.0.0.8", "127.0.0.1");

    /**
     * 鉴权入口：依据 X-Forwarded-For 判定是否放行管理操作。
     *
     * @param request 用户可控 HTTP 请求
     */
    public void handle(HttpRequest request) {
        // [VULN] 危险：直接取用户可控请求头作为来源 IP 鉴权依据
    ...
    public static void main(String[] args) {
        new XForwardedForAuthzVuln().handle(new HttpRequest("10.0.0.8"));
    }
}
