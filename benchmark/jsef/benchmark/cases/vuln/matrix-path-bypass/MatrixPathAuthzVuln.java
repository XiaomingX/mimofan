 ...
 * LLM 容易只关注一端状态机而误判为 SAFE。
 *
 * CWE-863 (Incorrect Authorization)。
 * 安全底线：仅 localhost 演示语义，不提供真实利用脚本。
 *
 * 修复要点（对照 MatrixPathAuthzSafe.java）：先按 UrlPathHelper / PathContainer
 * 归一化（去分号段）再做精确白名单匹配，且鉴权不依赖资源后缀。
 */
public class MatrixPathAuthzVuln {
    ...
    /**
     * 过滤器先按后缀跳过鉴权，框架再做分号段归一化 → 鉴权被绕过。
     *
     * @param rawPath 用户可控的原始请求路径
     */
    public boolean authorize(String rawPath) {
        if (rawPath.endsWith(".css")) {              // 过滤器：静态资源 → 跳过鉴权
            // [CHECKPOINT id=JSEF-MATRIX-001 cwe=863 level=L3 source=/admin;x.css style path sink=authorization decision after suffix-match skip expect=VULN trace=benchmark/cases/vuln/matrix-path-bypass/MatrixPathAuthzVuln.java:31,benchmark/cases/vuln/matrix-path-bypass/MatrixPathAuthzVuln.java:33,benchmark/cases/vuln/matrix-path-bypass/MatrixPathAuthzVuln.java:36]
            return true;                             // [VULN] 放行静态资源（= 鉴权被绕过）
        }
