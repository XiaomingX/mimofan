package com.jsef.benchmark.sec.samlxsw;
...
public class SamlXswSafe {
    ...
    /**
     * 仅返回 SignedInfo 实际引用并验签通过的 <Assertion>。
     * 语义等价 opensaml：从 XMLSignature 引用解引用出的节点读取，而非 document 第一个 Assertion。
     */
    ...
    public boolean authorize(String samlResponse) throws Exception {
        ...
        // [CHECKPOINT id=JSEF-SAML-001S cwe=347 level=L5 source=attacker-controlled SAML response sink=authorization based on SignedInfo-referenced assertion expect=SAFE]
        return grantAccess(asserted); // 只基于签名覆盖的断言授权
    }

    public static void main(String[] args) throws Exception {
        new SamlXswSafe().authorize("<samlp:Response>...</samlp:Response>");
    }
}
