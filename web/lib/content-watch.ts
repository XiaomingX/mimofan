...
const LINK_TARGETS: { url: string; label: string }[] = [
  ...
  { url: "https://github.com/Hmbown/homebrew-mimofan", label: "Homebrew tap" },
  ...
  { url: "https://registry.npmjs.org/mimofan", label: "npm package (registry API)" },
...
const SEMANTIC_DRIFT_PROMPT = `You are reviewing copy on a community website (mimofan.net) for the open-source Mimofan project.
...
export async function runSemanticDrift(env: WatchEnv): Promise<{ ok: boolean; drafted: number; reason?: string }> {
  ...
  const ghHeaders: Record<string, string> = {
    ...
    "User-Agent": "mimofan-web-semantic-drift",
  };
  ...
  const [changelog, commits, homepageHtml, docsHtml] = await Promise.all([
    ...
    fetch("https://mimofan.net/en", { headers: { "User-Agent": "mimofan-watch" } }).then((r) => r.ok ? r.text() : "").catch(() => ""),
    fetch("https://mimofan.net/en/docs", { headers: { "User-Agent": "mimofan-watch" } }).then((r) => r.ok ? r.text() : "").catch(() => ""),
  ]);
...
${docsText}`;
  ...
  for (const d of drifts) {
    ...
    const draft: AgentDraft = {
      id,
      type: "triage",
      targetUrl: `https://mimofan.net/en/${d.page === "homepage" ? "" : d.page}`,
      bodyEn: body,
      bodyZh: body,
      generatedAt: new Date().toISOString(),
      posted: false,
