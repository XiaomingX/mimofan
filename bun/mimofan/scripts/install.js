function assertSupportedNode() {
  ...
  if (Number.isNaN(major) || major < 18) {
    process.stderr.write(
      "mimofan: Node.js 18 or newer is required for bun installation. " +
      `Current Node.js version is ${version}. ` +
      "Please upgrade Node.js and rerun `bun install -g mimofan`.\n",
    );
    process.exit(1);
  }
}
...
const OPTIONAL_TIMEOUT_MS = 15_000; // fail fast during optional bun postinstall
...
// Optional install only relaxes bun postinstall behavior. Runtime downloads
...
function logInfo(message) {
  ...
  process.stderr.write(`mimofan: ${message}\n`);
}
...
function installFailureHint(error) {
  ...
  return [
    "mimofan install hint:",
    "  The bun package downloads prebuilt binaries from GitHub Releases.",
    "  If GitHub is unavailable on this network, mirror the release assets and set:",
    "    MIMOFAN_RELEASE_BASE_URL=https://<mirror>/<release-asset-directory>/",
    "  The directory must contain mimofan-artifacts-sha256.txt and the platform binaries.",
    "  See docs/INSTALL.md#bun-binary-download-times-out.",
  ].join("\n");
}
...
function createProgressReporter(assetName, totalBytes) {
  ...
  const render = (final) => {
    if (totalBytes && totalBytes > 0) {
      ...
      const line = `mimofan: downloading ${assetName}: ${formatMb(received)} / ${formatMb(totalBytes)} MB (${pct}%)`;
      if (interactive) {
      ...
      const line = `mimofan: downloading ${assetName}: ${formatMb(received)} MB downloaded`;
      if (interactive) {
        process.stderr.write(`${line}\r`);
      } else {
        process.stderr.write(`${line}\n`);
  ...
  return {
    ...
    finish() {
      ...
      process.stderr.write(`mimofan: ${assetName} ... done.\n`);
    },
  };
}
    ...
    if (totalTimeoutMs > 0) {
      totalTimer = setTimeout(() => {
        fail(new DownloadTimeoutError(
          ...
          `(set MIMOFAN_DOWNLOAD_TIMEOUT_MS to raise it; current stall budget is ${stallMs} ms)`,
        ));
      }, totalTimeoutMs);
    }
    ...
    const armStallTimer = () => {
      ...
      stallTimer = setTimeout(() => {
        fail(new DownloadTimeoutError(
          ...
          `(set MIMOFAN_DOWNLOAD_STALL_MS to raise it; total budget is ${totalTimeoutMs} ms)`,
        ));
      }, stallMs);
    };
...
async function downloadText(url, options = {}) {
  ...
  return withRetry(`fetch ${url}`, async () => {
    ...
    // this manifested as an empty checksum manifest in the bun wrapper
...
async function run(options = {}) {
  ...
  if (shouldSkipOptionalPostinstall(context)) {
    logInfo(
      "bun optional postinstall detected; skipping install-time download. The binary will be checked on first run.",
    );
    return;
  }
...
if (require.main === module) {
  run({ context: "install" }).catch((error) => {
    console.error("mimofan install failed:", error.message);
