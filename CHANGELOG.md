...
[Semantic Versioning](https://semver.org/) and are bumped from the workspace
root (`Cargo.toml` → `[workspace.package] version`).

## [0.0.4] - 2026-07-28

### Changed
- **Unified the project version to `0.0.4`.** `npm/mimofan` and
  `npm/runtime-sdk` package versions now track the Cargo workspace version
  (`[workspace.package] version` in `Cargo.toml`) instead of drifting on
  independent `0.8.x` lines. `scripts/release/prepare-release.sh` now bumps
  `npm/runtime-sdk` too, and `scripts/release/check-versions.sh` guards it.

### Fixed
- Cleaned up rebrand (`deepseek-tui` → `mimofan`) leftovers:
  restored `DEEPSEEK_*` environment-variable fallback chains that had been
  duplicated into `MIMOFAN_*` by the bulk rename, corrected legacy npm
  package references in the release tooling, and migrated the JS toolchain
  from pnpm to bun (`package.json` `workspaces` / `overrides` /
  `trustedDependencies`, generated `bun.lock`).
