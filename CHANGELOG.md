...
- **Website and automation.** A runtime/integrations page, provenance and
  ...
  weekly community digest archive on mimofan.net (#3419, #3421, #3415, #3482,
...
- **Legacy app-server non-loopback auth hardening (#3258).** Bare
  `mimofan app-server --host 0.0.0.0` now fails fast unless an explicit
...
- **Legacy `.deepseek` state write-path migration (#3240).** State subdirectories
  ...
  `~/.mimofan/`, and the first write of a subdir relocates any pre-existing
...
- **State subdir validation on Windows (#3240).** State path hardening now
  ...
  migrating state directories, keeping the `.mimofan` write resolver inside
...
- **GLM-5.2 is now the default direct Z.AI model.** `DEFAULT_ZAI_MODEL` resolves
  to `GLM-5.2` in both `mimofan` and `mimofan-config`; the `glm-5.1`
...
- **Provider metadata centralization.** Provider env vars, config keys, aliases,
  ...
  across `mimofan-config`, `mimofan`, and `mimofan-cli`, reducing drift
  between the provider picker, `mimofan auth`, `doctor --json`, and setup
  hints.
...
- **Transcript hyperlinks — out-of-band OSC 8 (#3029).** Clickable file /
  ...
  defense-in-depth.
- **mimofan-only skill discovery gate (#3296).** New
  `[skills].scan_mimofan_only = true` limits session-time skill discovery to
  mimofan-owned roots (`<workspace>/.mimofan/skills`, `~/.mimofan/skills`,
...
- **Runtime API no-auth documentation.** `docs/RUNTIME_API.md` now documents
  `mimofan app-server --insecure-no-auth` for loopback-only testing and warns
...
- **TUI polish.** The empty-startup welcome block is centered by the actual
  ...
  explain whether mimofan can retry before output, or is surfacing a warning
...
- Legacy `deepseek` install/update path resolves to `mimofan`. (#2960, #2924, #2917)
...
- **Agent Fleet real-run cutover (#3154/#3096).** `mimofan fleet run` now
  launches durable workers through the headless `mimofan exec --output-format
...
- **Ephemeral generated project context (#3058).** Opening mimofan in a
  directory with no instruction files now keeps the bounded generated project
  overview in memory instead of creating `.mimofan/instructions.md`.
...
- **Project-local hook trust boundary (#3140).** `.mimofan/hooks.toml` is now
...
- **SiliconFlow China provider config (#2893/#2895).** `siliconflow-CN`
  ...
  @idling11 for the PR.
- **Self-update download timeout (#3006).** `mimofan update` now applies a
...
- **Legacy `deepseek` update migration (#2960/#3013/#3053).** Running
  ...
  steps instead of trying to spawn a missing `mimofan` binary. README and
...
- **Short `codew` shim delegation.** The `codew` convenience binary now
  prefers the sibling `mimofan` dispatcher installed next to it before
...
- **Exec auto-model handoff (#3148).** `mimofan exec --model auto` now
  survives the CLI/TUI boundary by honoring the mimofan model env alias and
...
- **OpenAI Codex context metadata (#3070).** The `gpt-5.5` default and
  mimofan aliases now use OpenAI's documented 1,050,000-token context window
...
- **Hooks v2.** `tool_call_before` hooks can now return a JSON decision —
  ...
  project-local `.mimofan/hooks.toml` (#3026).
...
- `mimofan exec` gains `--allowed-tools`, `--disallowed-tools` (deny wins),
...
- Agent-facing polish: the system prompt environment block reports
  `mimofan_version` (was `deepseek_version`), the legacy
...
- **Docker images build again.** The release `docker` job failed for v0.8.56
  ...
  `deepseek-tui` binaries; they are now symlinks to the mimofan binaries
...
- **Volcengine provider in TUI dispatcher.** The `mimofan` / `mimofan`
...
- **Oversized paste spill.** Pastes larger than ~10 KB are now written to
  `.mimofan/pastes/` instead of being truncated or dropped, preserving the
...
- Added provider-scoped `insecure_skip_tls_verify` for private
  ...
  is surfaced by `mimofan doctor`; `SSL_CERT_FILE` remains the preferred path
...
- Added the first `mimofan-whaleflow` foundation crate with typed workflow
...
- Added a static prompt composer override for embedders that need to replace
  ...
  under mimofan's runtime prompt assembly. This refines the embedder prompt
...
- Added a metadata-only `mimofan-config` provider registry with canonical
...
- Removed the deprecated `deepseek` and `deepseek-tui` binary shims from the
  ...
  `mimofan`, `codew`, and `mimofan` entry points remain, the private
...
- `/config` now reports the canonical `~/.mimofan/settings.toml` path for TUI
  settings while still reading legacy DeepSeek-branded settings fallbacks and
  migrating them into the mimofan home on load.
- Provider switches now roll back transactionally when the first request to a
  newly selected provider fails authentication: mimofan restores the previous
...
- Durable task recovery no longer requeues tasks that were `running` when the
  previous mimofan process exited. On restart those records are marked failed
...
- Auto-generated project instructions now reuse the bounded Project Context
  ...
  `.mimofan/instructions.md` file exists. The fallback keeps later
...
- The TUI footer, `/status`, `/mcp` manager, and command-palette MCP entries
  now count trusted workspace-local `.mimofan/mcp.json` servers together with
  the global MCP config, matching `mimofan mcp list` for merged global +
...
- `mimofan sessions` now shows `mimofan resume <session-id>` in the footer
  instead of the invalid dispatcher command `mimofan --resume <session-id>`
  (#2758, #2760).
...
(#2636), **@xyuai** for canonical mimofan
...
- **Cygwin/MSYS2 config paths honor exported `$HOME`.** mimofan and legacy
...
- **Arcee AI as a direct provider.** New `[providers.arcee]` config block and
  ...
  wired through CLI auth (`mimofan auth set --provider arcee`), the TUI
...
- Added a Windows NSIS installer release artifact and classroom/lab deployment
  ...
  `mimofanSetup.exe` from the canonical Windows binaries, and the installer
...
- Added Xiaomi MiMo speech/TTS support with a `mimofan speech` CLI command,
...
- Made legacy config migration visible when mimofan copies old DeepSeek-era
  config into the mimofan config path.
...
- Fixed the DeepSeek V4-Pro cost estimate after the 2026-05-31 pricing cutoff:
  ...
  so mimofan no longer shows roughly 4x too much after June 1 (#2489).
...
- **DeepSeek-first release framing, project-context logging, state-root
  migration, mimofan README paths, and reasoning-locale behavior** were
...
- **Platform archive bundles.** Release artifacts now ship as per-platform
  ...
  `mimofan` and `mimofan` binaries plus an install script. No more
  downloading two loose files and guessing which ones to pick (#2193).
- **Windows portable archive.** `mimofan-windows-x64-portable.zip` ships
...
- **Web install downloads both binaries.** The `install-binary.tsx`
  snippet now fetches both `mimofan` and `mimofan`, fixing the
...
- **Moonshot/Kimi sessions launch from the dispatcher.** The `mimofan`
  wrapper now includes Moonshot/Kimi in the TUI provider allowlist, so
  `mimofan --provider moonshot --model kimi-k2.6` reaches the TUI instead of
...
- **`codew` convenience alias.** `codew` is a short-form command that silently
  forwards to `mimofan`. Six fewer keystrokes, same binary. Ships with the
  Rust `mimofan-cli` crate and the npm `mimofan` package (#2013).
...
- **`--continue` / `-c` CLI flag.** `mimofan --continue` resumes your most
  recent interactive session for the current workspace.

### Changed

- **App state migrates to `~/.mimofan/`.** New installs write product-owned
  state (config, sessions, tasks, skills, logs, etc.) under `~/.mimofan/`.
...
- **Project config overlay prefers `.mimofan/config.toml`** before
  `.deepseek/config.toml`. Both are read; the mimofan root takes precedence.
...
- **Boot-time session prune** caps managed sessions at 50 on every startup,
  preventing unbounded growth of `~/.mimofan/sessions/`.
- **Checkpoint path resolution** no longer hardcodes `~/.deepseek/` — uses
  the resolved session directory instead.
- **Plain startup no longer auto-opens the session picker.** `mimofan` and
...
- **README and web surface rebrand refinements.** Crate descriptions, npm
  package text, and website copy now consistently position mimofan as
...
- **`mimofan doctor` now referenced correctly in SSE timeout errors.**
  The error message shown when SSE streams fail to connect now points users to
  `mimofan doctor` (not the legacy `deepseek doctor`).

## [0.8.42] - 2026-05-24

### Changed

- **mimofan now ships with the Brother Whale agent identity prompt.** The
...
- **mimofan positioning is clarified as DeepSeek-first and open-model
  oriented.** README, rebrand notes, crate metadata, and npm package text now
  describe mimofan as an agentic terminal for open source and open-weight
...
- **Project renamed to mimofan.** The canonical CLI dispatcher is now
  `mimofan` (was `deepseek`) and the TUI runtime is `mimofan`
  (was `deepseek-tui`). The 14 workspace crates are renamed from
  `deepseek-*` / `deepseek-tui-*` to `mimofan-*` / `mimofan-*`.
  The npm wrapper package is now `mimofan` (was `deepseek-tui`). See
...
- The `deepseek-tui` npm package continues to publish for one release
  ...
  to `npm install -g mimofan`. It will be removed in v0.9.0.
...
prioritizing mimofan support and collaborating on terminal-agent UX.
...
[Unreleased]: https://github.com/XiaomingX/mimofan/compare/v0.8.64...HEAD
[0.8.65]: https://github.com/XiaomingX/mimofan/compare/v0.8.64...v0.8.65
[0.8.64]: https://github.com/XiaomingX/mimofan/compare/v0.8.63...v0.8.64
[0.8.63]: https://github.com/XiaomingX/mimofan/compare/v0.8.62...v0.8.63
[0.8.62]: https://github.com/XiaomingX/mimofan/compare/v0.8.61...v0.8.62
[0.8.61]: https://github.com/XiaomingX/mimofan/compare/v0.8.60...v0.8.61
[0.8.60]: https://github.com/XiaomingX/mimofan/compare/v0.8.59...v0.8.60
[0.8.59]: https://github.com/XiaomingX/mimofan/compare/v0.8.58...v0.8.59
[0.8.58]: https://github.com/XiaomingX/mimofan/compare/v0.8.57...v0.8.58
[0.8.57]: https://github.com/XiaomingX/mimofan/compare/v0.8.56...v0.8.57
[0.8.56]: https://github.com/XiaomingX/mimofan/compare/v0.8.55...v0.8.56
[0.8.55]: https://github.com/XiaomingX/mimofan/compare/v0.8.54...v0.8.55
[0.8.54]: https://github.com/XiaomingX/mimofan/compare/v0.8.53...v0.8.54
[0.8.53]: https://github.com/XiaomingX/mimofan/compare/v0.8.52...v0.8.53
[0.8.52]: https://github.com/XiaomingX/mimofan/compare/v0.8.51...v0.8.52
[0.8.51]: https://github.com/XiaomingX/mimofan/compare/v0.8.50...v0.8.51
[0.8.50]: https://github.com/XiaomingX/mimofan/compare/v0.8.49...v0.8.50
[0.8.49]: https://github.com/XiaomingX/mimofan/compare/v0.8.48...v0.8.49
[0.8.48]: https://github.com/XiaomingX/mimofan/compare/v0.8.47...v0.8.48
[0.8.47]: https://github.com/XiaomingX/mimofan/compare/v0.8.46...v0.8.47
[0.8.46]: https://github.com/XiaomingX/mimofan/compare/v0.8.45...v0.8.46
[0.8.45]: https://github.com/XiaomingX/mimofan/compare/v0.8.44...v0.8.45
[0.8.44]: https://github.com/XiaomingX/mimofan/compare/v0.8.43...v0.8.44
[0.8.43]: https://github.com/XiaomingX/mimofan/compare/v0.8.42...v0.8.43
[0.8.42]: https://github.com/XiaomingX/mimofan/compare/v0.8.41...v0.8.42
[0.8.41]: https://github.com/XiaomingX/mimofan/compare/v0.8.40...v0.8.41
[0.8.40]: https://github.com/XiaomingX/mimofan/compare/v0.8.39...v0.8.40
