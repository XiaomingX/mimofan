...
impl CustomAgentRegistry {
    ...
    fn scan_dir(dir: &Path, agents: &mut HashMap<String, CustomAgentDef>) {
...
/// ---
/// ```
fn parse_agent_file(path: &Path) -> Result<Option<CustomAgentDef>> {
...
mod tests {
    ...
    fn test_parse_agent_file_with_frontmatter() {
        ...
        let agent = parse_agent_file(&file_path).expect("parse agent file").expect("parse agent file");
