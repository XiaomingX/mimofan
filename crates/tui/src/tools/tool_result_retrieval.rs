...
//! `~/.mimofan/tool_outputs/<tool-call-id>.txt` by `tools::truncate`. This
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "ref": {
                    "type": "string",
                    "description": "Tool call id, artifact id (`art_<id>`), SHA ref (`sha:<64-hex>`), spillover filename, or absolute path under ~/.mimofan."
                },
                "mode": {
                    "type": "string",
...
fn resolve_spillover_reference(reference: &str, session_id: &str) -> Result<PathBuf, ToolError> {
    let root = crate::tools::truncate::spillover_root()
        .ok_or_else(|| ToolError::execution_failed("could not resolve ~/.mimofan/tool_outputs"))?;
    let root_canonical = root.canonicalize().ok();

    // Resolve the session's `artifacts/` directory.
    // `session_artifact_absolute_path(sid, p)` returns
    // `~/.mimofan/sessions/<sid>/<p>` — so passing the literal
    ...
    let try_path = |candidate: PathBuf, tried: &mut Vec<PathBuf>| -> Option<PathBuf> {
        ...
        // home-level `~/.mimofan/tool_outputs/` dir is engine-only and
