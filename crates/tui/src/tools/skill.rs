...
impl ToolSpec for LoadSkillTool {
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        // can't find.
        let discovery_mode =
            SkillDiscoveryMode::from_mimofan_only(context.skills_scan_mimofan_only);
...
mod tests {
    ...
    #[tokio::test]
    async fn execute_respects_mimofan_only_skill_discovery() {
        let tmp = tempdir().unwrap();
        let workspace = tmp.path().to_path_buf();
        write_skill(
        ...
        let mimofan_dir = workspace.join(".mimo").join("skills");
        write_skill(
            &mimofan_dir,
            "mimofan-only",
            "mimofan skill",
        ...
        let context = ToolContext::new(workspace).with_skills_config(mimofan_dir, true);
        let tool = LoadSkillTool;

        let result = tool
            .execute(json!({"name": "mimofan-only"}), &context)
            .await
            .expect("mimofan skill should load");
        assert!(result.success);

        let err = tool
            .execute(json!({"name": "claude-only"}), &context)
            .await
            .expect_err("Claude skill should be hidden in mimofan-only mode");
        let msg = err.to_string();
        assert!(
            msg.contains("claude-only") && msg.contains("mimofan-only"),
