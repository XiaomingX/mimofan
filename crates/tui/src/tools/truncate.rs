    ...
    F: FnOnce() -> R,
{
    ...
    // artifacts.rs tests.
    let prior_spillover =
        set_test_spillover_root(Some(home.join(".mimofan").join(SPILLOVER_DIR_NAME)));
    let prior_artifacts = crate::artifacts::set_test_artifact_sessions_root(Some(
        home.join(".mimofan").join("sessions"),
    ));
...
mod tests {
    ...
    fn with_test_home_overrides_storage_roots_without_home_resolution() {
        ...
        with_test_home(tmp.path(), || {
            assert_eq!(
                ...
                Some(tmp.path().join(".mimofan").join("tool_outputs").as_path())
            );
            assert_eq!(
    ...
    fn write_spillover_creates_directory_and_writes_file() {
        ...
        with_test_home(tmp.path(), || {
            ...
            // Directory landed under `<HOME>/.mimofan/tool_outputs/`.
            ...
            let components: Vec<&str> = path
                ...
                .collect();
            assert!(
                components.contains(&".mimofan") && components.contains(&"tool_outputs"),
                "spillover path missing expected `.mimofan/tool_outputs/...` segments: {path:?}"
            );
        });
    }
    ...
    fn apply_spillover_with_artifact_writes_session_file_and_ref_block() {
        ...
        with_test_home(tmp.path(), || {
            ...
            assert!(
                tmp.path()
                    .join(".mimofan/tool_outputs/call-big.txt")
                    .exists(),
