//! `mimofan remote-setup` — guided generation of a remote-agent deploy bundle.
//!
...
//! [`registry::BRIDGES`], and the existing `mimofan_config::provider` registry
...
/// Flags for `mimofan remote-setup` (clap), per the RFC command surface.
...
pub fn run_remote_setup(args: RemoteSetupArgs) -> Result<()> {
    ...
    let out_dir = args.out.clone().unwrap_or_else(|| {
        PathBuf::from("mimofan-deploy").join(format!("{}-{}", cloud.slug, bridge.slug))
    });
...
fn resolve_provider(args: &RemoteSetupArgs) -> Result<ProviderInfo> {
    if let Some(slug) = &args.provider {
        return ProviderInfo::from_slug(slug).ok_or_else(|| {
            anyhow::anyhow!(
                ...
                mimofan_config::ProviderKind::names_hint()
            )
        });
    }
    if args.non_interactive {
    ...
    let providers: Vec<ProviderInfo> = mimofan_config::ProviderKind::all()
        .iter()
