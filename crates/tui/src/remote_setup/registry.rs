//! Table-driven registries for `mimofan remote-setup`.
//!
...
//!   `mimofan_config::provider` registry (see [`super::bundle::ProviderInfo`]).

/// Where a cloud target stores the runtime/provider secrets.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SecretStore {
    /// Secrets live in `/etc/mimofan/*.env` files on the host.
    EnvFile,
...
impl SecretStore {
    ...
    pub fn label(self) -> &'static str {
        match self {
            SecretStore::EnvFile => "EnvFile (/etc/mimofan/*.env)",
...
pub const TELEGRAM: BridgeSpec = BridgeSpec {
    ...
    service_unit: "mimofan-telegram-bridge.service",
    ...
    install_dir: "/opt/mimofan/telegram-bridge",
};
...
pub const FEISHU: BridgeSpec = BridgeSpec {
    ...
    service_unit: "mimofan-feishu-bridge.service",
    ...
    install_dir: "/opt/mimofan/bridge",
};
...
/// `/etc/mimofan/*.env`, native+systemd install driven by a cloud-init
...
fn lighthouse_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    let restart_bridge = format!("mimofan-{}-bridge", inputs.bridge_slug);
    vec![
        ProvisionStep::new(
...
fn azure_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    let provider_secret = format!("mimofan-{}-key", inputs.provider_slug);
    vec![
        ProvisionStep::new(
...
fn digitalocean_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    vec![
        ...
        ProvisionStep::new(
            "On the Droplet: write /etc/mimofan/*.env, install both systemd units, enable --now",
            "bash",
            &["scripts/tencent-lighthouse/install-services.sh"],
        ),
    ]
