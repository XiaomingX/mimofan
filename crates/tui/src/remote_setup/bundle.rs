...
//!
//! Env files lead with `MIMOFAN_*` keys; `MIMOFAN_*` are documented as legacy
...
pub struct ProviderInfo {
    ...
    pub display: String,
    /// The provider's own API-key env var, e.g. `"MIMOFAN_API_KEY"` (env_keys[0]).
    pub key_var: String,
...
fn render_bridge_env(i: &BundleInputs) -> String {
    ...
    out.push_str(
        "# Transport only: forwards chat <-> the local runtime. No provider keys here.\n\n",
    );
...
ExecStart=/bin/sh -lc 'exec /home/mimofan/.cargo/bin/mimofan serve --http --host 127.0.0.1 --port \"${{MIMOFAN_RUNTIME_PORT:-{port}}}\" --workers \"${{MIMOFAN_RUNTIME_WORKERS:-{workers}}}\" --auth-token \"${{MIMOFAN_RUNTIME_TOKEN}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
