...
ExecStart=/bin/sh -lc 'exec /home/mimofan/.cargo/bin/mimofan serve --http --host 127.0.0.1 --port \"${{CODEWHALE_RUNTIME_PORT:-${{DEEPSEEK_RUNTIME_PORT:-{port}}}}}\" --workers \"${{CODEWHALE_RUNTIME_WORKERS:-${{DEEPSEEK_RUNTIME_WORKERS:-{workers}}}}}\" --auth-token \"${{CODEWHALE_RUNTIME_TOKEN:-${{DEEPSEEK_RUNTIME_TOKEN}}}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
...
mod tests {
    ...
    fn bundle_renders_expected_file_set() {
        ...
        assert!(names.contains(&"mimofan-runtime.service"));
        assert!(names.contains(&"mimofan-feishu-bridge.service"));
    ...
    #[test]
    fn env_files_lead_with_mimofan_keys() {
    ...
    #[test]
    fn systemd_units_reference_mimofan_paths() {
        let inputs = sample_inputs(&LIGHTHOUSE, &FEISHU, "deepseek");
        let files = render_bundle(&inputs);
        let unit = &files
            .iter()
            .find(|f| f.relative_path == "mimofan-runtime.service")
            .unwrap()
            .contents;
        assert!(unit.contains("/etc/mimofan/runtime.env"));
        ...
        let bridge_unit = &files
            .iter()
            .find(|f| f.relative_path == "mimofan-feishu-bridge.service")
            .unwrap()
            .contents;
        assert!(bridge_unit.contains("/etc/mimofan/feishu-bridge.env"));
    }
}
