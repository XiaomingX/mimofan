//! Bundle rendering for `mimofan remote-setup`.
//!
...
//!
//! Env files lead with `MIMOFAN_*` keys; `DEEPSEEK_*` are documented as legacy
...
/// `mimofan_config::provider` registry (the single source of truth).
#[derive(Debug, Clone)]
...
impl ProviderInfo {
    ...
    pub fn from_slug(slug: &str) -> Option<Self> {
        let kind = mimofan_config::ProviderKind::parse(slug)?;
        let p = mimofan_config::provider::provider_for_kind(kind);
        let key_var = p.env_vars().first().copied().unwrap_or("MIMOFAN_API_KEY");
        Some(Self {
            slug: p.id().to_string(),
...
impl BundleInputs {
    ...
    pub fn deploy_inputs(&self) -> DeployInputs {
        DeployInputs {
            ...
            instance_name: "mimofan-remote".to_string(),
            image: "ghcr.io/hmbown/mimofan:latest".to_string(),
        }
    }
}
...
pub fn render_bundle(inputs: &BundleInputs) -> Vec<RenderedFile> {
    vec![
        ...
        RenderedFile {
            relative_path: "mimofan-runtime.service".to_string(),
...
fn render_runtime_env(i: &BundleInputs) -> String {
    ...
    out.push_str("# mimofan runtime config — generated by `mimofan remote-setup`.\n");
    out.push_str("# Provider configuration lives here; the bridge is pure transport.\n");
    out.push_str("# MIMOFAN_* keys are canonical. DEEPSEEK_* are read as legacy aliases.\n\n");

    out.push_str(&format!("MIMOFAN_PROVIDER={}\n", i.provider.slug));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "MIMOFAN_MODEL={}   # provider default is {}\n",
    ...
    out.push_str(&format!("MIMOFAN_RUNTIME_TOKEN={}\n", i.runtime_token));
    out.push_str(&format!("MIMOFAN_RUNTIME_PORT={}\n", i.port));
    out.push_str(&format!("MIMOFAN_RUNTIME_WORKERS={}\n", i.workers));
...
fn render_bridge_env(i: &BundleInputs) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "# mimofan {} bridge config — generated by `mimofan remote-setup`.\n",
        i.bridge.display
    ));
    ...
    out.push_str(&format!("MIMOFAN_RUNTIME_URL={DEFAULT_RUNTIME_URL}\n"));
    out.push_str("# Must match MIMOFAN_RUNTIME_TOKEN in runtime.env.\n");
    out.push_str(&format!("MIMOFAN_RUNTIME_TOKEN={}\n", i.runtime_token));
    out.push_str(&format!("MIMOFAN_WORKSPACE={}\n", i.workspace));
    out.push_str(&format!("MIMOFAN_MODEL={}\n", i.model));
    out.push_str("MIMOFAN_MODE=agent\n");
    out.push_str("MIMOFAN_ALLOW_SHELL=true\n");
    out.push_str("MIMOFAN_TRUST_MODE=false\n");
    out.push_str("MIMOFAN_AUTO_APPROVE=false\n\n");
    ...
    out.push_str("\n# --- bridge tuning ---\n");
    out.push_str(&format!(
        "{}_THREAD_MAP_PATH=/var/lib/mimofan-{}-bridge/thread-map.json\n",
    ...
    out.push_str("MIMOFAN_TURN_TIMEOUT_MS=900000\n");
    out
}
...
fn render_runtime_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=mimofan Runtime API\n\
...
# Legacy /etc/deepseek is loaded first for old installs; /etc/mimofan wins.\n\
EnvironmentFile=-/etc/deepseek/runtime.env\n\
EnvironmentFile=-/etc/mimofan/runtime.env\n\
ExecStart=/bin/sh -lc 'exec /home/mimofan/.cargo/bin/mimofan serve --http --host 127.0.0.1 --port \"${{MIMOFAN_RUNTIME_PORT:-${{DEEPSEEK_RUNTIME_PORT:-{port}}}}}\" --workers \"${{MIMOFAN_RUNTIME_WORKERS:-${{DEEPSEEK_RUNTIME_WORKERS:-{workers}}}}}\" --auth-token \"${{MIMOFAN_RUNTIME_TOKEN:-${{DEEPSEEK_RUNTIME_TOKEN}}}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
...
fn render_bridge_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=mimofan {display} Phone Bridge\n\
Wants=network-online.target mimofan-runtime.service\n\
After=network-online.target mimofan-runtime.service\n\n\
[Service]\n\
Type=simple\n\
User=mimofan\n\
Group=mimofan\n\
...
EnvironmentFile=-/etc/mimofan/{slug}-bridge.env\n\
...
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/var/lib/mimofan-{slug}-bridge\n\n\
[Install]\n\
WantedBy=multi-user.target\n",
...
fn render_runbook(i: &BundleInputs) -> String {
    ...
    out.push_str(&format!(
        "# mimofan remote-setup runbook — {} + {}\n\n",
        i.cloud.display, i.bridge.display
    ));
    out.push_str("Generated by `mimofan remote-setup` (generate-only). Nothing was run on\n");
    ...
    out.push_str("| `mimofan-runtime.service` | systemd unit for the runtime API. |\n");
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "- A random `MIMOFAN_RUNTIME_TOKEN` was generated (`{}`). It is identical in\n",
        i.runtime_token
    ));
    ...
    out.push_str("```sh\nsudo install -d -m 750 /etc/mimofan\n");
    out.push_str(&format!(
        "sudo install -m 600 runtime.env /etc/mimofan/runtime.env\n\
sudo install -m 600 {slug}.env /etc/mimofan/{slug}-bridge.env\n\
sudo install -m 644 mimofan-runtime.service /etc/systemd/system/mimofan-runtime.service\n\
sudo install -m 644 {unit} /etc/systemd/system/{unit}\n\
sudo systemctl daemon-reload\n\
sudo systemctl enable --now mimofan-runtime {unit}\n```\n\n",
        slug = i.bridge.slug,
    ...
    out.push_str("```sh\nsudo systemctl status mimofan-runtime --no-pager\n");
    out.push_str(&format!(
