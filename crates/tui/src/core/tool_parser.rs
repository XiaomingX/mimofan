...
fn get_xml_tool_call_regex() -> &'static Regex {
    XML_TOOL_CALL_REGEX.get_or_init(|| {
        // Match <mimo:tool_call>...</mimo:tool_call> or similar XML patterns
        Regex::new(r"(?s)<(?:mimo:)?tool_call[^>]*>\s*(.*?)\s*</(?:mimo:)?tool_call>")
...
pub fn parse_tool_calls(text: &str) -> ParseResult {
    ...
    // Parse XML-style <mimo:tool_call> or <tool_call> format
