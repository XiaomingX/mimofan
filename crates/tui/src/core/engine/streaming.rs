...
/// explains why mimofan may retry before any output and why it must surface
...
pub(super) fn stream_read_error_user_message(message: &str, any_content_received: bool) -> String {
    ...
    let retry_note = if any_content_received {
        "Some output had already streamed, so mimofan is surfacing the warning instead of replaying the request and risking duplicated output."
    } else {
        "No output had streamed yet, so mimofan will retry automatically while retry budget remains."
    };
    format!(
