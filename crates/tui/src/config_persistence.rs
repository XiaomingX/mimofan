...
pub(crate) fn persist_limits_integer_key(
    config_path: Option<&Path>,
    key: &str,
    value: u64,
) -> anyhow::Result<PathBuf> {
    ...
    let value = i64::try_from(value).context("integer value is too large for TOML")?;
    persist_limits_value_key(config_path, key, toml::Value::Integer(value))
}

fn persist_limits_value_key(
    config_path: Option<&Path>,
    key: &str,
    value: toml::Value,
) -> anyhow::Result<PathBuf> {
    ...
    let table = doc
        .as_table_mut()
        .context("config.toml root must be a table")?;
    let limits_entry = table
        .entry("limits".to_string())
        .or_insert_with(|| toml::Value::Table(toml::value::Table::new()));
    let limits_table = limits_entry
        .as_table_mut()
        .context("`limits` section in config.toml must be a table")?;
    limits_table.insert(key.to_string(), value);
...
fn provider_base_url_table_key(provider: ApiProvider) -> anyhow::Result<&'static str> {
    match provider {
        ApiProvider::OpenAiCompatible => Ok("openai_compatible"),
        ApiProvider::AnthropicCompatible => Ok("anthropic_compatible"),
        ApiProvider::GeminiCompatible => Ok("gemini_compatible"),
    }
}
