...
mod tests {
                ...
                &serde_json::json!({
                    "task_updates": {
                        "gate": {
                            "id": "gate_test",
                            "gate": "test",
                            "command": "cargo test -p mimofan-tui --lib",
                            "cwd": ".",
                            "exit_code": 0,
                            "status": "passed",
    ...
    #[test]
    fn default_tasks_dir_prefers_existing_mimofan_tasks() {
        let temp_home = tempfile::tempdir().unwrap();
        let home = temp_home.path();
        let primary_tasks = home.join(".mimofan").join("tasks");
        let legacy_tasks = home.join(".deepseek").join("tasks");
    ...
    #[test]
    fn default_tasks_dir_uses_mimofan_tasks_for_new_installs() {
        let temp_home = tempfile::tempdir().unwrap();
        let home = temp_home.path();

        assert_eq!(
            default_tasks_dir_for_home(home),
            home.join(".mimofan").join("tasks")
        );
    }
}
