...
pub(crate) fn read_text_with_wlpaste_using_argv(program: &str) -> Result<String> {
...
pub(crate) fn osc52_sequence(text: &str, in_tmux: bool) -> Result<String> {
...
pub(crate) fn clipboard_images_dir_for_home(workspace: &Path, home: Option<&Path>) -> PathBuf {
    if let Some(home) = home {
        return home.join(".mimofan").join("clipboard-images");
    }
...
pub(crate) fn save_image_as_png_in(dir: &Path, image: &ImageData) -> Result<PastedImage> {
