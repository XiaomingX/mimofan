//! Self-update for the `mimofan` binary.
//!
//! The `update` subcommand fetches the latest release from
//! `github.com/XiaomingX/mimofan/releases/latest`, downloads the
...
pub fn run_update(
    ...
    if check_only {
        ...
        if update_is_needed(channel, current_version, &latest_tag)? {
            println!("Update available. Run `mimofan update` to install {latest_tag}.");
        } else {
...
pub(crate) fn legacy_binary_message(current_exe: &Path) -> String {
    format!(
        "\
...
pub(crate) struct UpdateTarget {
    ...
    pub(crate) asset_stem: String,
}

pub(crate) fn update_targets_for_exe(current_exe: &Path) -> Vec<UpdateTarget> {
...
pub(crate) fn select_platform_asset<'a>(
    release: &'a Release,
    binary_name: &str,
) -> Option<&'a Asset> {
    release
...
pub(crate) fn parse_checksum_manifest(text: &str) -> Result<HashMap<String, String>> {
...
pub(crate) fn expected_sha256_from_manifest(text: &str, asset_name: &str) -> Result<String> {
...
pub(crate) fn release_from_mirror_base_url(
    base_url: &str,
    version: &str,
    os: &str,
    rust_arch: &str,
...
pub(crate) fn release_from_github_download_tag(
    tag_name: &str,
    os: &str,
    rust_arch: &str,
) -> Release {
...
pub(crate) fn fetch_latest_release_from_url(url: &str, proxy: Option<&Proxy>) -> Result<Release> {
...
pub(crate) fn fetch_latest_stable_tag_from_redirect_url(
    url: &str,
    proxy: Option<&Proxy>,
) -> Result<String> {
    let client = update_http_client(proxy)?;
...
pub(crate) fn release_tag_from_github_release_url(url: &reqwest::Url) -> Option<String> {
...
pub(crate) fn release_tag_from_github_release_html(body: &str) -> Option<String> {
    const MARKERS: &[&str] = &[
        "/XiaomingX/mimofan/releases/tag/",
        "/xiaomingx/mimofan/releases/tag/",
        "/releases/tag/",
    ];
...
pub(crate) fn fetch_latest_beta_release_from_url(
    url: &str,
    proxy: Option<&Proxy>,
) -> Result<Release> {
    let body = fetch_release_json(url, "release list", proxy)?;
    // GitHub caps this endpoint at 100 releases per page. mimofan uses the
...
pub(crate) fn download_url(url: &str, proxy: Option<&Proxy>) -> Result<Vec<u8>> {
    let mut last_error = None;
    for attempt in 1..=UPDATE_HTTP_ATTEMPTS {
...
pub(crate) fn parse_glibc_version(text: &str) -> Option<GlibcVersion> {
...
pub(crate) fn highest_required_glibc(bytes: &[u8]) -> Option<GlibcVersion> {
...
pub(crate) fn glibc_compatibility_message(
    asset_name: &str,
...
pub(crate) fn replace_binary(target: &Path, new_bytes: &[u8]) -> Result<()> {
    let parent = target
        .parent()
