//! `mimofan metrics` — reads the audit log and session/task stores and prints
...
/// Arguments accepted by `mimofan metrics`.
...
impl Rollup {
    ...
    pub fn tool_mut(&mut self, name: &str) -> &mut ToolStats {
...
pub fn read_audit_log(path: &Path, since: Option<DateTime<Utc>>, rollup: &mut Rollup) {
