...
mod tests {
    ...
    #[test]
    fn test_global_commands_dir_contains_mimofan_commands() {
        ...
        assert!(
            parts
                .windows(2)
                .any(|pair| pair == [".mimofan", "commands"]),
            "expected .mimofan/commands components in path, got: {}",
            dir.display()
        );
    }
    ...
    fn load_user_commands_scans_workspace_local_dir() {
        ...
        let ws = tmp.path();
        let cmds_dir = ws.join(".mimofan").join("commands");
    ...
    fn workspace_local_shadows_global_by_name() {
        ...
        assert_eq!(
            ...
            "workspace-local (.mimofan) must shadow later dirs"
        );
    }
    ...
    fn new_user_command_clears_stale_paused_state() {
        ...
        let tmp = TempDir::new().unwrap();
        let ws = tmp.path().to_path_buf();
        let commands_dir = ws.join(".mimofan").join("commands");
        write_command(
            &commands_dir,
            "pause-scan",
            "---\ndescription: Scan repos\npausable: true\n---\nscan",
