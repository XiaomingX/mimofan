...
//!
//! - **Load** `~/.mimofan/memory.md` (path is configurable via
//!   `memory_path` in `config.toml` and `MIMOFAN_MEMORY_PATH` env),
...
//! `[memory] enabled = true` in `config.toml` or `MIMOFAN_MEMORY=on`.
