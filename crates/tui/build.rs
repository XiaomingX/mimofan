...
fn main() {
    println!("cargo:rerun-if-env-changed=MIMOFAN_BUILD_SHA");
    ...
    println!("cargo:rustc-env=MIMOFAN_BUILD_VERSION={build_version}");
}
...
fn configure_windows_stack() {
    ...
    match std::env::var("CARGO_CFG_TARGET_ENV").as_deref() {
        Ok("msvc") => {
            println!("cargo:rustc-link-arg-bin=mimofan=/STACK:8388608");
        }
        Ok("gnu") => {
            println!("cargo:rustc-link-arg-bin=mimofan=-Wl,--stack,8388608");
        }
        _ => {}
    }
}
