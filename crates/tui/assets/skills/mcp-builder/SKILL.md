---
name: mcp-builder
description: Design, build, configure, or debug Model Context Protocol servers for mimofan, including stdio and HTTP/SSE transports.
---
...
HTTP/SSE entries can include per-server headers in `~/.mimofan/mcp.json` when
...
4. Add the server with `mimofan mcp add` or edit `~/.mimofan/mcp.json`.
